import type { Runtype, Static } from 'funtypes'
import {
  InstanceOf,
  Literal,
  Never,
...
const uint16 = Num.withConstraint(
  x => x === (x & 0xffff) >>> 0 || `expected ${x} to be an unsigned 16-bit integer`,
  {
    name: 'Uint16',
  },
)
...
const uint8 = Num.withConstraint(
  x => x === (x & 0xff) >>> 0 || `expected ${x} to be an unsigned 8-bit integer`,
  {
    name: 'Uint8',
  },
)
...
export function assertType<T extends Runtype<unknown>>(
  rt: T,
  value: unknown,
...
function byteArrayBase<T extends NonNegativeInteger | undefined = undefined>(length?: T) {
  ...
  return (length === undefined ? indefinite : definite) as Ternary<
    T,
    typeof definite,
    typeof indefinite
  >
