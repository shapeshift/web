import { arbitrumNova } from '@shapeshiftmonorepo/chain-adapters'
...
export const http = new unchained.arbitrumNova.V1Api(
  new unchained.arbitrumNova.Configuration({
    basePath: getConfig().VITE_UNCHAINED_ARBITRUM_NOVA_HTTP_URL,
  }),
)
            ...
            [
              ...
              () => {
                const ws = new unchained.ws.Client<unchained.arbitrumNova.Tx>(
                  getConfig().VITE_UNCHAINED_ARBITRUM_NOVA_WS_URL,
                )

                return new arbitrumNova.ChainAdapter({
                  providers: { http, ws },
                  rpcUrl: getConfig().VITE_ARBITRUM_NOVA_NODE_URL,
                })
              },
            ],
          ],
