...
import type { CipherString } from './classes'
import { EncryptedObject, SymmetricCryptoKey } from './classes'
import type { CryptoEngine } from './engines'
...
export default class CryptoHelper {
  ...
  async pbkdf2(
    ...
    password =
      typeof password !== 'string' ? password : core.toArrayBuffer(utils.fromUtf8ToArray(password))
  ...
  async makeKey(password: string, email: string): Promise<SymmetricCryptoKey> {
    ...
    const key = await this.#engine.scrypt(
      core.toArrayBuffer(utils.fromUtf8ToArray(password)),
      salt,
      {
        iterations: 16384,
        blockSize: 8,
