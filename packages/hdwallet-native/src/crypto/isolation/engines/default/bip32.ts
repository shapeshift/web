  ...
  implements Core.BIP32.Node, Core.SecP256K1.ECDSARecoverableKey, Core.SecP256K1.ECDHKey
{
  ...
  async getPublicKey(): Promise<Core.SecP256K1.CompressedPoint> {
    this.#publicKey =
      this.#publicKey ??
      checkType(Core.SecP256K1.CompressedPoint, ecc.pointFromScalar(this.#privateKey, true))
  ...
  async ecdsaSignRecoverable(
    ...
    return await Core.SecP256K1.RecoverableSignature.fromSignature(
      checkType(
        Core.SecP256K1.Signature,
        ecc.sign(Buffer.from(msgOrDigest), this.#privateKey, entropy),
      ),
      null,
      msgOrDigest,
  ...
  private async _ecdh(
    ...
    digestAlgorithm === undefined ||
      digestAlgorithm === null ||
      Core.Digest.AlgorithmName(32).assert(digestAlgorithm)
