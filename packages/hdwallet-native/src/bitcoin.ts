...
export function MixinNativeBTCWalletInfo<TBase extends core.Constructor<core.HDWalletInfo>>(
  Base: TBase,
) {
...
export function MixinNativeBTCWallet<TBase extends core.Constructor<NativeHDWalletBase>>(
  Base: TBase,
) {
  // eslint-disable-next-line @typescript-eslint/no-shadow
        ...
        switch (scriptType) {
          ...
          case 'bech32':
            scriptData.redeemScript = payment.redeem?.output
              ? Buffer.from(payment.redeem.output)
              : undefined
            break
        }
          ...
          outputs.map(async output => {
            ...
            try {
              ...
              if (output.address !== undefined) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                const keyPair = await util.getKeyPair(
                  this.#masterKey!,
                  output.addressNList,
                  coin,
                  output.scriptType,
