  ...
  get: {
    'https://dex.binance.org/api/v1/node-info': {
      ...
      validator_info: {
        ...
        pub_key: [
          113, 242, 215, 184, 236, 28, 139, 153, 166, 83, 66, 155, 1, 24, 205, 32, 31, 121, 79, 64,
          157, 15, 234, 77, 101, 177, 182, 98, 242, 176, 0, 99,
        ],
...
describe('NativeBinanceWallet', () => {
  ...
  it('should generate another correct binance address', async () => {
    expect(
      await wallet.binanceGetAddress({
        addressNList: core.bip32ToAddressNList("m/44'/714'/1337'/123/4"),
      }),
  ...
  it('should only handle pubkeys returned from the BNB SDK if they are in amino format', async () => {
    ...
    const mock = vi
      ...
      .mockImplementation(async function (this: any, ...args: any[]) {
        ...
        expect(out.signatures[0].pub_key.toString('base64')).toEqual(
          'A/7/niWhEmyNrOL5jDyKiFyN1fMYjCxVPgyucVvj9UNT',
        )
        return out
      })
    await expect(
