...
const RFOX_FIRST_EPOCH_START_TIMESTAMP = BigInt(dayjs().subtract(30, 'days').unix())
...
export const fetchEpochHistory = async (): Promise<EpochMetadata[]> => {
  ...
  while (startTimestamp < now) {
    const nextStartTimestamp = BigInt(dayjs.unix(Number(startTimestamp)).add(30, 'days').unix())
