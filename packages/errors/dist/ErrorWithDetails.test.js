...
describe('ErrorWithDetails', () => {
    it('should create an error with no message', () => {
        const e = new ErrorWithDetails_1.ErrorWithDetails();
        expect(e).toBeInstanceOf(ErrorWithDetails_1.ErrorWithDetails);
        expect(e.message).toBe('');
        expect(e.cause).toBeUndefined();
    });
    it('should create an error with a message', () => {
        const e = new ErrorWithDetails_1.ErrorWithDetails('test message');
        expect(e.message).toBe('test message');
        expect(e.cause).toBeUndefined();
    });
    it('should create an error with a message and a cause', () => {
        const cause = new TypeError('cause');
        const e = new ErrorWithDetails_1.ErrorWithDetails('test message', { cause });
        expect(e.message).toBe('test message');
        expect(e.cause).toBe(cause);
    });
    it('should create an error with a message and details', () => {
        const details = { prop: true };
        const e = new ErrorWithDetails_1.ErrorWithDetails('test message', { details });
        expect(e.message).toBe('test message');
        expect(e.cause).toBeUndefined();
    ...
    it('should ignore unknown properties in options object', () => {
        const cause = new TypeError('cause');
        const e = new ErrorWithDetails_1.ErrorWithDetails('test message', { cause, stuff: { arg1: 'foo' } });
        expect(e.message).toBe('test message');
        expect(e.cause).toBe(cause);
    ...
    it('should support an error code', () => {
        ...
        const e2 = new ErrorWithDetails_1.ErrorWithDetails('test message', { code: 'ERR_test' });
