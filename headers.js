...
const cspMeta = Object.entries({
  ...
  'connect-src': [
    "'self'",
    'data:',
    // Fixes local (dev) websocket requests in Safari on macOS/iOS
    ...(process.env.NODE_ENV === 'development' ? ['ws://localhost:3000/ws',
    `ws://${process.env.REACT_APP_LOCAL_IP}:3000/ws`] : []),
