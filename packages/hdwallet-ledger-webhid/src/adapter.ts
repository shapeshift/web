...
export class WebHIDLedgerAdapter {
  ...
  constructor(keyring: core.Keyring) {
    ...
    if (window && window.navigator.hid) {
      ...
      window.navigator.hid.addEventListener(
        'disconnect',
        this.handleDisconnectWebHIDLedger.bind(this),
      )
    }
  }
  ...
  private async handleConnectWebHIDLedger(e: HIDConnectionEvent): Promise<void> {
    ...
    try {
      ...
      this.keyring.emit(
        ['Ledger', e.device.productName ?? '', core.Events.CONNECT],
        MOCK_SERIAL_NUMBER,
      )
    } catch (error: any) {
      this.keyring.emit(
  ...
  private async handleDisconnectWebHIDLedger(e: HIDConnectionEvent): Promise<void> {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        this.keyring.emit(
          ['Ledger', e.device.productName ?? '', core.Events.DISCONNECT],
          MOCK_SERIAL_NUMBER,
        )
      }
  ...
  public async initialize(device?: HIDDevice): Promise<number> {
    ...
    if (device) {
      ...
      const wallet = ledger.create(
        new LedgerWebHIDTransport(device, ledgerTransport as Transport, this.keyring),
      )
