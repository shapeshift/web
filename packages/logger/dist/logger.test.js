...
describe('Logger', () => {
    ...
    describe('constructor', () => {
        it('should set logger name and level', () => {
            ...
            expect(() => {
                logger = new logger_1.Logger({
                    name: 'testLogger',
                    level: logger_type_1.LogLevel.TRACE,
                });
            }).not.toThrow();
        ...
        it('should accept "namespace" as a string', () => {
            ...
            const logger = new logger_1.Logger({ namespace: 'testLogger' });
            logger.info('test');
        ...
        it('should accept "namespace" as an array', () => {
            console.info = jest.fn();
            const logger = new logger_1.Logger({ namespace: ['testLogger', 'child'] });
            logger.info('test');
            expect(console.info).toHaveBeenCalledWith(expect.stringMatching('namespace":"testLogger:child"'));
        });
        it('should accept "namespace" and "name" at the same time', () => {
            const spy = jest.spyOn(console, 'info');
            const logger = new logger_1.Logger({ namespace: ['testLogger', 'child'], name: 'child2' });
            logger.info('test');
            expect(spy).toHaveBeenCalledWith(expect.stringMatching('namespace":"testLogger:child:child2"'));
            spy.mockRestore();
        });
        it('should accept default log level as an enum', () => {
            console.debug = jest.fn();
            const logger = new logger_1.Logger({ level: logger_type_1.LogLevel.DEBUG });
            logger.debug('test');
        ...
        it('should accept default log level as a string', () => {
            console.debug = jest.fn();
            const logger = new logger_1.Logger({ level: 'debug' });
            logger.debug('test');
            expect(console.debug).toHaveBeenCalled();
            // @ts-ignore
            expect(logger.level).toBe(1);
        ...
        it('should throw an error if log level is invalid', () => {
            expect(() => new logger_1.Logger({ level: 'silly' })).toThrowError('logging level');
        });
        it('should disable logging if LogLevel is NONE', () => {
            const spy = jest.spyOn(console, 'debug');
            const logger = new logger_1.Logger({ level: logger_type_1.LogLevel.NONE });
            logger.trace('test');
        ...
        it('should allow for an empty namespace', () => {
            ...
            logger.info('test');
            expect(spy).toHaveBeenCalledWith('{"message":"test","timestamp":"2020-01-01T00:00:00Z","status":"info"}');
            spy.mockRestore();
        });
        it('should support a custom logger function', () => {
            const logFn = jest.fn();
            const logger = new logger_1.Logger({ name: 'test', logFn });
            logger.info('test');
    ...
    describe('Logger functions', () => {
        ...
        let logger;
        beforeEach(() => {
            logger = new logger_1.Logger({ name, level: logger_type_1.LogLevel.TRACE, defaultFields: { default: true } });
        });
        ...
        describe('.child(...x: any[])', () => {
            ...
            it('should override the log level of the parent', () => {
                const traceSpy = jest.spyOn(console, 'debug').mockReturnValue(undefined);
                logger.child({ name: 'info', level: logger_type_1.LogLevel.WARN }).trace('child');
            ...
            it('should pass through the logFn of the parent', () => {
                const logFn = jest.fn();
                const parent = new logger_1.Logger({ name: 'test', logFn });
                parent.child({ name: 'info' }).info('child');
                expect(logFn).toHaveBeenCalledWith('info', expect.objectContaining({ status: 'info' }));
            });
            it('should include child keys', () => {
                logger.child({ foo: 'bar' }).info('something');
                const expected = '{"default":true,"foo":"bar","message":"something","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger","status":"info"}';
                expect(spy).toHaveBeenCalledTimes(1);
                expect(spy).toHaveBeenCalledWith(expected);
            });
            it('should include grandchild keys', () => {
                logger.child({ foo: 'bar' }).child({ zan: ':)' }).info('something');
                const expected = '{"default":true,"foo":"bar","zan":":)","message":"something","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger","status":"info"}';
                expect(spy).toHaveBeenCalledTimes(1);
                expect(spy).toHaveBeenCalledWith(expected);
            });
            it('should add to the namespace from "name" and "namespace"', () => {
                logger
                    ...
                    .info('something');
                const expected = '{"default":true,"message":"something","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger:foo:bar:deep:deeper:last","status":"info"}';
                expect(spy).toHaveBeenCalledTimes(1);
                expect(spy).toHaveBeenCalledWith(expected);
            });
            it("should use parent namespace if new namespace isn't provided", () => {
                logger.child().info('something');
                const expected = '{"default":true,"message":"something","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger","status":"info"}';
                expect(spy).toHaveBeenCalledTimes(1);
                expect(spy).toHaveBeenCalledWith(expected);
            });
        });
        describe('.trace(x: object, msg: string)', () => {
            it('should log an object and message', () => {
                ...
                expect(spy).toHaveBeenCalledWith('{"default":true,"foo":"bar","message":"message","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger","status":"trace"}');
                spy.mockRestore();
            });
        ...
        describe.each([logger_type_1.LogLevel.DEBUG, logger_type_1.LogLevel.INFO, logger_type_1.LogLevel.WARN, logger_type_1.LogLevel.ERROR])('.%s(x: object, msg: string)', level => {
            it('should log an object and message', () => {
                // @ts-ignore
                const spy = jest.spyOn(console, level).mockReturnValue(undefined);
                // @ts-ignore
                logger[level]({ foo: 'bar' }, 'message');
                expect(spy).toHaveBeenCalledWith(`{"default":true,"foo":"bar","message":"message","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger","status":"${level}"}`);
                spy.mockRestore();
            });
            it('should throw a validation error', () => {
                // @ts-ignore
        ...
        it('should allow for multiple string arguments', () => {
            ...
            expect(spy).toHaveBeenCalledWith('{"default":true,"message":"message","timestamp":"2020-01-01T00:00:00Z","namespace":"testLogger","status":"info","_messages":["some unexpected error as a string","another","message"]}');
            spy.mockRestore();
        });
    });
});
