...
export const isBlastChainAdapter = (adapter: unknown): adapter is EvmChainAdapter => {
  if (!adapter) return false

  const maybeAdapter = adapter as EvmChainAdapter
  if (typeof maybeAdapter.getChainId !== 'function') return false

  return maybeAdapter.getChainId() === blastChainId
}

export const getBlastTransactionStatus = async (
  txHash: string,
  nodeUrl: string,
): Promise<TxStatus> => {
  try {
    ...
    console.error('[Blast] Error getting transaction status:', error)
    return TxStatus.Unknown
  }
}
