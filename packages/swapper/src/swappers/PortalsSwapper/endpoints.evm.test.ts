...
import { portalsApi } from './endpoints'
...
describe('portals getUnsignedEvmTransaction', () => {
  it('builds from step.transactionData (evm), not portalsTransactionMetadata', async () => {
    const buildCustomApiTx = vi.fn(async (a: any) => a)
    const adapter = {
      buildCustomApiTx,
    }
    const tx = await portalsApi.getUnsignedEvmTransaction!({
      from: '0xFROM',
      stepIndex: 0,
