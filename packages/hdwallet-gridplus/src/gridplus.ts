...
const isSafecardConnected = (uid?: Buffer): boolean =>
  !!uid && `0x${uid.toString('hex')}` !== zeroHash
    ...
    core.ThorchainWalletInfo
{
  ...
  btcGetAccountPaths(msg: core.BTCGetAccountPaths): core.BTCAccountPath[] {
    ...
    const coinPaths: Record<string, core.BTCAccountPath[]> = {
  ...
  btcNextAccountPath(msg: core.BTCAccountPath): core.BTCAccountPath | undefined {
    ...
    if (
      (addressNList[0] === 0x80000000 + 44 &&
        msg.scriptType == core.BTCInputScriptType.SpendAddress) ||
      (addressNList[0] === 0x80000000 + 49 &&
        msg.scriptType == core.BTCInputScriptType.SpendP2SHWitness) ||
      (addressNList[0] === 0x80000000 + 84 &&
        msg.scriptType == core.BTCInputScriptType.SpendWitness)
    ) {
      addressNList[2] += 1
    ...
    core.ThorchainWallet
{
  ...
  async getPublicKeys(msg: core.GetPublicKey[]): Promise<(core.PublicKey | null)[]> {
    ...
    const publicKeys: (core.PublicKey | null)[] = []
      ...
      try {
        ...
        let xpub =
          typeof addresses[0] === 'string'
            ? addresses[0]
            : Buffer.from(addresses[0]).toString('hex')
