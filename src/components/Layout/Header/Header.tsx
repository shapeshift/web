...
const WalletConnectToDappsHeaderButton = lazy(() =>
  import(
    "@/plugins/walletConnectToDapps/components/header/WalletConnectToDappsHeaderButton"
  ).then(({ WalletConnectToDappsHeaderButton }) => ({
    default: WalletConnectToDappsHeaderButton,
  })),
);
...
export const Header = memo(() => {
  ...
  const isWalletConnectToDappsV2Enabled = useFeatureFlag(
    "WalletConnectToDappsV2",
  );
  ...
  const isOrderlyPerpetualsEnabled = useFeatureFlag("OrderlyPerpetuals");
  const { degradedChainIds } = useDiscoverAccounts();

  const tradeSubMenuItems = useMemo(() => {
    const items = [...baseTradeSubMenuItems];
    if (isOrderlyPerpetualsEnabled) {
      items.push(perpetualsMenuItem);
    }
    return items;
  }, [isOrderlyPerpetualsEnabled]);
    ...
    <>
      ...
      >
        <HStack height="4.5rem" width="full" pr={4} pl={6}>
          ...
          <Box
            maxW={searchBoxMaxWSx}
            minW={searchBoxMinWSx}
            mx={4}
          ...
          >
            ...
            {isLargerThanMd &&
              (isDegradedState || degradedChainIds.length > 0) && (
                <DegradedStateBanner />
              )}
