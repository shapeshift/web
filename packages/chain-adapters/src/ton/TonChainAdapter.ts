...
const supportsTon = (wallet: HDWallet): wallet is TonWallet => {
  return (
    "_supportsTon" in wallet && (wallet as TonWallet)._supportsTon === true
  );
};
...
export class ChainAdapter implements IChainAdapter<KnownChainIds.TonMainnet> {
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          if (response.status === 429) {
            ...
            const backoffDelay =
              Math.min(retryAfter * 1000, 10000) * Math.pow(1.5, attempt);
  ...
  async getAccount(pubkey: string): Promise<Account<KnownChainIds.TonMainnet>> {
    try {
      ...
      try {
        const accountInfo = await this.rpcRequest<TonAccountInfo>(
          ...
          tokens = jettonsResponse.jetton_wallets
            ...
            .map((jw) => {
              ...
              const jettonUserFriendly =
                addressBook[jettonRawAddress]?.user_friendly ??
                jettonRawAddress;
  ...
  async buildSendTransaction(
    input: BuildSendTxInput<KnownChainIds.TonMainnet>,
  ): Promise<{
    txToSign: TonSignTx;
  }> {
    try {
  ...
  async signAndBroadcastTransaction({
    ...
    try {
      const signedTx = await this.signTransaction(
        signTxInput as SignTxInput<TonSignTx>,
      );
  ...
  async broadcastTransaction(
    ...
    try {
      ...
      const result = await this.rpcRequest<{ hash: string }>(
        "sendBocReturnHash",
        {
          boc: signedTx,
        },
  ...
  async getFeeData(
    ...
    try {
      ...
      const fastFee = (
        (BigInt(totalFee) * BigInt(150)) /
        BigInt(100)
      ).toString();
      const slowFee = (
        (BigInt(totalFee) * BigInt(80)) /
        BigInt(100)
      ).toString();
