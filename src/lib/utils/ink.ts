...
export const isInkChainAdapter = (adapter: unknown): adapter is EvmChainAdapter => {
  if (!adapter) return false

  const maybeAdapter = adapter as EvmChainAdapter
  if (typeof maybeAdapter.getChainId !== 'function') return false

  return maybeAdapter.getChainId() === inkChainId
}

export const getInkTransactionStatus = async (txHash: string): Promise<TxStatus> => {
  const rpcUrl = getConfig().VITE_INK_NODE_URL
  ...
  try {
    ...
    console.error('Error fetching Ink transaction status:', error)
    return TxStatus.Unknown
  }
}
