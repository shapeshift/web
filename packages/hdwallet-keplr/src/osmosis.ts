...
export function osmosisGetAccountPaths(msg: OsmosisGetAccountPaths): OsmosisAccountPath[] {
  return [
    {
      addressNList: [
        0x80000000 + 44,
        0x80000000 + slip44ByCoin('Osmo'),
...
export async function osmosisGetAddress(provider: any): Promise<string | undefined> {
  const offlineSigner = provider.getOfflineSigner('osmosis-1')
