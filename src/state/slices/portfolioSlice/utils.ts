  ...
  Object.entries(args.portfolioAccounts).forEach(([_xpubOrAccount, account]) => {
    ...
    switch (chain) {
      ...
      case ChainTypes.Ethereum: {
        ...
        const AccountId = caip10.toCAIP10({ caip2: chainId, account: _xpubOrAccount })
        ...
        portfolio.accounts.byId[accountSpecifier].push(assetId)
        ...
        ethAccount.chainSpecific.tokens?.forEach(token => {
          ...
          portfolio.accounts.byId[AccountId].push(token.assetId)
          // add assetId without dupes
      ...
      case ChainTypes.Bitcoin: {
        ...
        portfolio.accounts.byId[accountSpecifier] = Array.from(
          new Set([...portfolio.accounts.byId[accountSpecifier], assetId]),
        )
        ...
        addresses.forEach(({ pubkey }) => {
          const accountId = caip10.toCAIP10({ caip2: chainId, account: pubkey })
