...
export async function ethSignTypedData(
  ...
  const res = await (async () => {
    const _res = await transport.call(
      'Eth',
      'signEIP712Message',
      bip32path,
      msg.typedData as EIP712Message,
    )
...
export async function ethVerifyMessage(msg: core.ETHVerifyMessage): Promise<boolean> {
  ...
  const buffer = isBytes(msg.message)
    ? Buffer.from(arrayify(msg.message))
  ...
  return (
    core.stripHexPrefixAndLower(msg.address) === ethereumUtil.pubToAddress(pubKey).toString('hex')
  )
}
