...
import {
  getStarknetTransactionStatus,
  isStarknetChainAdapter,
...
export const useSendActionSubscriber = () => {
  ...
  const completeAction = useCallback(
    (action: ReturnType<typeof selectPendingWalletSendActions>[number]) => {
      const { txHash, accountId, accountIdsToRefetch } =
      ...
      const isSecondClassChain = SECOND_CLASS_CHAINS.includes(
  ...
  useEffect(() => {
    pendingSendActions.forEach((action) => {
      ...
      const isChainWithoutTxHistory = SECOND_CLASS_CHAINS.includes(
        chainId as KnownChainIds,
      );
            ...
            try {
              ...
              switch (chainId) {
                ...
                case KnownChainIds.SuiMainnet: {
                  ...
                  isConfirmed =
                    suiTxStatus === TxStatus.Confirmed ||
                    suiTxStatus === TxStatus.Failed;
                  break;
                }
                case KnownChainIds.MonadMainnet: {
                  const monadTxStatus = await getMonadTransactionStatus(txHash);
                  isConfirmed =
                    monadTxStatus === TxStatus.Confirmed ||
                    monadTxStatus === TxStatus.Failed;
                  break;
                }
                case KnownChainIds.PlasmaMainnet: {
                  const plasmaTxStatus =
                    await getPlasmaTransactionStatus(txHash);
                  isConfirmed =
                    plasmaTxStatus === TxStatus.Confirmed ||
                    plasmaTxStatus === TxStatus.Failed;
                  break;
                }
                case KnownChainIds.HyperEvmMainnet: {
                  ...
                  isConfirmed =
                    hyperEvmTxStatus === TxStatus.Confirmed ||
                    hyperEvmTxStatus === TxStatus.Failed;
                  break;
                }
                case KnownChainIds.NearMainnet: {
                  const nearTxStatus = await getNearTransactionStatus(txHash);
                  isConfirmed =
                    nearTxStatus === TxStatus.Confirmed ||
                    nearTxStatus === TxStatus.Failed;
                  break;
                }
                case KnownChainIds.StarknetMainnet: {
                  ...
                  if (isStarknetChainAdapter(adapter)) {
                    ...
                    if (starknetTxStatus === TxStatus.Failed) {
                      ...
                      try {
                        if (adapter?.parseTx) {
                          const parsedTx = await adapter.parseTx(
                            txHash,
                            accountAddress,
                          );
                          dispatch(
                        ...
                        console.error(
                          "Failed to parse failed Starknet Tx:",
                          error,
                        );
                      }
                      ...
                      const intervalId =
                        pollingIntervalsRef.current.get(pollingKey);
                      if (intervalId) {
                ...
                case KnownChainIds.ThorchainMainnet: {
                  const thorTxStatus =
                    await getThorchainSendTransactionStatus(txHash);
                  isConfirmed =
                    thorTxStatus === TxStatus.Confirmed ||
                    thorTxStatus === TxStatus.Failed;
                  break;
                }
                case KnownChainIds.MayachainMainnet: {
                  const mayaTxStatus =
                    await getMayachainTransactionStatus(txHash);
                  isConfirmed =
                    mayaTxStatus === TxStatus.Confirmed ||
                    mayaTxStatus === TxStatus.Failed;
                  break;
                }
                default:
