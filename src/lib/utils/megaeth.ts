...
export const isMegaEthChainAdapter = (adapter: unknown): adapter is EvmChainAdapter => {
  if (!adapter) return false

  const maybeAdapter = adapter as EvmChainAdapter
  if (typeof maybeAdapter.getChainId !== 'function') return false

  return maybeAdapter.getChainId() === megaethChainId
}
...
export const getMegaEthTransactionStatus = async (txHash: string): Promise<TxStatus> => {
  ...
  try {
    ...
    console.error('Error fetching MegaETH transaction status:', error)
    return TxStatus.Unknown
  }
}
