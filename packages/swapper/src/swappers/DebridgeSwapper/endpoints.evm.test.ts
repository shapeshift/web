...
import { debridgeApi } from './endpoints'
...
describe('debridge getUnsignedEvmTransaction', () => {
  it('builds from step.transactionData (evm), not debridgeTransactionMetadata', async () => {
    const buildCustomApiTx = vi.fn(async (a: any) => a)
    const adapter = { buildCustomApiTx }
    const tx = await debridgeApi.getUnsignedEvmTransaction!({
      from: '0xFROM',
      stepIndex: 0,
