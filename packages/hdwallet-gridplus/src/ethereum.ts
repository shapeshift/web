...
import { Constants, Utils } from 'gridplus-sdk'

export async function ethGetAddress(
  client: Client,
  msg: core.ETHGetAddress,
): Promise<core.Address | null> {
...
export async function ethSignTx(client: Client, msg: core.ETHSignTx): Promise<core.ETHSignedTx> {
  ...
  const payload = isEIP1559
    ...
    : RLP.encode(unsignedTx.getMessageToSign())
  ...
  const decoderResult = await (() => {
    if (
      !msg.data ||
      (msg.data.startsWith('0x') && Buffer.from(msg.data.slice(2), 'hex').length < 4)
    ) {
