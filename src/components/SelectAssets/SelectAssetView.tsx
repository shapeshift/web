...
export const SelectAssetView = ({ onClick, toRoute, assetId }: SelectAssetViewProps) => {
  ...
  const handleAssetSelect = (asset: Asset) => {
    ...
    assetAccounts && assetAccounts.length > 1 ? navigate(SelectAssetRoutes.Account, { state: { assetId: asset.caip19 } }) : (() => handleAccountSelect(asset, assetAccounts[0]))
  ...
  <Routes location={location} key={location.key}>
    ...
    <Route
      ...
      element={<SelectAccount onClick={handleAccountSelect} />}
    />
    <Route path='/' element={() => <Navigate to={SelectAssetRoutes.Search} />} />
  </Routes>)
}
