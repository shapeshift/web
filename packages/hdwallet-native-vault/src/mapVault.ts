...
import type { IVaultBackedBy, IVaultFactory, VaultPrepareParams } from './types'
...
export class MapVault
  ...
  implements
    Map<string, unknown | Promise<unknown>>,
    IVaultBackedBy<[string, unknown | Promise<unknown>][]>
{
  ...
  async load(deserializer: (_: [string, unknown | Promise<unknown>][]) => Promise<void>) {
  ...
  async save(serializer: () => Promise<[string, unknown | Promise<unknown>][]>) {
    await this.#rawVault.save(async (): Promise<Uint8Array> => {
      const payloadObj = (
        await Promise.all(
          (await serializer()).map(async ([k, v]): Promise<[typeof k, unknown]> => [k, await v]),
        )
      )
        .sort((a, b) => {
          if (a[0] < b[0]) return -1
  ...
  async entriesAsync(): Promise<[string, unknown][]> {
