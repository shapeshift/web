...
describe('BigAmount', () => {
  ...
  describe('fromBaseUnit', () => {
    it('converts base unit to precision scale for BTC (8 decimals)', () => {
      const amount = BigAmount.fromBaseUnit('150000000', 8)
    ...
    it('converts base unit to precision scale for ETH (18 decimals)', () => {
      const amount = BigAmount.fromBaseUnit('1000000000000000000', 18)
    ...
    it('converts base unit to precision scale for USDC (6 decimals)', () => {
      const amount = BigAmount.fromBaseUnit('1500000', 6)
      expect(amount.toPrecision()).toBe('1.5')
      expect(amount.precision).toBe(6)
    })

    it('handles dust amounts', () => {
      const amount = BigAmount.fromBaseUnit('1', 18)
      expect(amount.toPrecision()).toBe('0.000000000000000001')
    })

    it('handles large numbers', () => {
      const amount = BigAmount.fromBaseUnit('999999999999999999', 18)
      expect(amount.toPrecision()).toBe('0.999999999999999999')
    })

    it('handles zero', () => {
      const amount = BigAmount.fromBaseUnit('0', 8)
  ...
  describe('fromPrecision', () => {
    it('stores precision-scale value directly', () => {
      const amount = BigAmount.fromPrecision('1.5', 8)
      expect(amount.toPrecision()).toBe('1.5')
      expect(amount.precision).toBe(8)
    })

    it('stores ETH amount directly', () => {
      const amount = BigAmount.fromPrecision('1', 18)
      expect(amount.toPrecision()).toBe('1')
    })
  ...
  describe('nullish-safe construction', () => {
    it('fromBaseUnit treats undefined as zero', () => {
      expect(BigAmount.fromBaseUnit(undefined, 8).isZero()).toBe(true)
    })

    it('fromBaseUnit treats null as zero', () => {
      expect(BigAmount.fromBaseUnit(null, 8).isZero()).toBe(true)
    })

    it('fromPrecision treats undefined as zero', () => {
      expect(BigAmount.fromPrecision(undefined, 8).isZero()).toBe(true)
    })

    it('fromPrecision treats null as zero', () => {
      expect(BigAmount.fromPrecision(null, 8).isZero()).toBe(true)
    })

    it('fromPrecision treats empty string as zero', () => {
      expect(BigAmount.fromPrecision('', 8).isZero()).toBe(true)
    })

    it('fromPrecision treats NaN as zero', () => {
      expect(BigAmount.fromPrecision(NaN, 8).isZero()).toBe(true)
    })

    it('fromBaseUnit treats NaN as zero', () => {
      expect(BigAmount.fromBaseUnit(NaN, 18).isZero()).toBe(true)
    })
  })

  describe('zero', () => {
    it('creates a zero amount at given precision', () => {
      const amount = BigAmount.zero(18)
      expect(amount.isZero()).toBe(true)
      expect(amount.precision).toBe(18)
  ...
  describe('fromBN', () => {
    it('wraps an existing BigNumber', () => {
      const amount = BigAmount.fromBN(bn('1.5'), 8)
      expect(amount.toPrecision()).toBe('1.5')
      expect(amount.precision).toBe(8)
    })

    it('treats non-finite BigNumber as zero', () => {
      const amount = BigAmount.fromBN(bn(Infinity), 8)
      expect(amount.isZero()).toBe(true)
    })
  })
  ...
  describe('base unit / precision safety', () => {
    it('fromBaseUnit and fromPrecision produce the same BigAmount for equivalent values', () => {
      const fromBase = BigAmount.fromBaseUnit('1000000000000000000', 18)
      const fromPrec = BigAmount.fromPrecision('1', 18)
    ...
    it('normalizes both to precision scale so operations are always safe', () => {
      const a = BigAmount.fromBaseUnit('150000000', 8)
      const b = BigAmount.fromPrecision('0.5', 8)
      const sum = a.plus(b)
    ...
    it('prevents the classic bug: accidentally adding wei to ETH', () => {
      const oneEthFromWei = BigAmount.fromBaseUnit('1000000000000000000', 18)
      const oneEthFromPrecision = BigAmount.fromPrecision('1', 18)
  ...
  describe('round-trip', () => {
    it.each([
      ...
      ({ baseUnit, precision }) => {
        const amount = BigAmount.fromBaseUnit(baseUnit, precision)
  ...
  describe('plus', () => {
    it('adds two BigAmounts with same precision', () => {
      const a = BigAmount.fromPrecision('1.5', 8)
      const b = BigAmount.fromPrecision('2.5', 8)
    ...
    it('throws when adding BigAmounts with different precisions', () => {
      const btc = BigAmount.fromPrecision('1', 8)
      const eth = BigAmount.fromPrecision('1', 18)
  ...
  describe('minus', () => {
    it('subtracts two BigAmounts with same precision', () => {
      const a = BigAmount.fromPrecision('5', 8)
      const b = BigAmount.fromPrecision('2', 8)
  ...
  describe('div', () => {
    it('divides by a scalar', () => {
      const amount = BigAmount.fromPrecision('10', 8)
      expect(amount.div(2).toPrecision()).toBe('5')
    })

    it('handles fractional division', () => {
      const amount = BigAmount.fromPrecision('1', 8)
      expect(amount.div(3).toFixed(8)).toBe('0.33333333')
    })

    it('throws if passed a BigAmount (dimensionally invalid)', () => {
      const a = BigAmount.fromPrecision('10', 8)
      const b = BigAmount.fromPrecision('2', 8)
  ...
  describe('abs', () => {
    it('returns absolute value', () => {
      const amount = BigAmount.fromPrecision('-5', 8)
  ...
  describe('positiveOrZero', () => {
    ...
    it('returns self for zero', () => {
      const amount = BigAmount.zero(8)
      expect(amount.positiveOrZero().isZero()).toBe(true)
    })
  })

  describe('withPrecision', () => {
    it('changes precision while preserving precision-scale value', () => {
      const btcAmount = BigAmount.fromPrecision('1.5', 8)
      const reExpressed = btcAmount.withPrecision(18)
      expect(reExpressed.toPrecision()).toBe('1.5')
      expect(reExpressed.precision).toBe(18)
      expect(reExpressed.toBaseUnit()).toBe('1500000000000000000')
    })

    it('maps to existing convertPrecision pattern', () => {
      const baseUnitAt8 = '150000000'
      const result = BigAmount.fromBaseUnit(baseUnitAt8, 8).withPrecision(18).toBaseUnit()
  ...
  describe('decimalPlaces', () => {
    it('rounds to specified decimal places', () => {
      const amount = BigAmount.fromPrecision('1.123456789', 18)
      expect(amount.decimalPlaces(4).toPrecision()).toBe('1.1235')
    })

    it('rounds down when specified', () => {
      const amount = BigAmount.fromPrecision('1.999', 18)
  ...
  describe('arithmetic chaining', () => {
    it('chains multiple operations with precision', () => {
      const result = BigAmount.fromBaseUnit('1000000000000000000', 18)
        .times(2500)
        .div(1.1)
        .toFixed(2)
    ...
    it('handles the common pattern: base unit → fiat display', () => {
      const fiat = BigAmount.fromBaseUnit('150000000', 8).times(60000).toFixed(2)
  ...
  describe('comparison', () => {
    it('gt with BigAmount', () => {
      const a = BigAmount.fromPrecision('2', 8)
      const b = BigAmount.fromPrecision('1', 8)
    ...
    it('lt', () => {
      const a = BigAmount.fromPrecision('1', 8)
      const b = BigAmount.fromPrecision('2', 8)
    ...
    it('eq', () => {
      const a = BigAmount.fromPrecision('1.5', 8)
      const b = BigAmount.fromPrecision('1.5', 8)
  ...
  describe('boolean checks', () => {
    it('isZero', () => {
      expect(BigAmount.fromPrecision('0', 8).isZero()).toBe(true)
      expect(BigAmount.fromPrecision('1', 8).isZero()).toBe(false)
    })

    it('isPositive', () => {
      expect(BigAmount.fromPrecision('1', 8).isPositive()).toBe(true)
      expect(BigAmount.fromPrecision('-1', 8).isPositive()).toBe(false)
      expect(BigAmount.fromPrecision('0', 8).isPositive()).toBe(true) // BigNumber considers 0 positive
    })

    it('isNegative', () => {
      expect(BigAmount.fromPrecision('-1', 8).isNegative()).toBe(true)
      expect(BigAmount.fromPrecision('1', 8).isNegative()).toBe(false)
    })

    it('isFinite', () => {
      expect(BigAmount.fromPrecision('1', 8).isFinite()).toBe(true)
      expect(BigAmount.fromPrecision(undefined, 8).isFinite()).toBe(true)
    })
  })
  ...
  describe('toBaseUnit', () => {
    it('converts to integer base unit string', () => {
      expect(BigAmount.fromPrecision('1.5', 8).toBaseUnit()).toBe('150000000')
    })

    it('converts ETH to wei', () => {
      expect(BigAmount.fromPrecision('1', 18).toBaseUnit()).toBe('1000000000000000000')
    })

    it('converts USDC to micro-units', () => {
      expect(BigAmount.fromPrecision('100.5', 6).toBaseUnit()).toBe('100500000')
    })

    it('converts zero', () => {
      expect(BigAmount.zero(8).toBaseUnit()).toBe('0')
    })
  })

  describe('toPrecision', () => {
    it('returns full precision string', () => {
      expect(BigAmount.fromBaseUnit('150000000', 8).toPrecision()).toBe('1.5')
    })

    it('is an alias for toString', () => {
      const amount = BigAmount.fromBaseUnit('123456789', 8)
  ...
  describe('toFixed', () => {
    it('returns fixed decimal places', () => {
      expect(BigAmount.fromPrecision('1.5', 8).toFixed(2)).toBe('1.50')
    })

    it('rounds down by default', () => {
      expect(BigAmount.fromPrecision('1.999', 8).toFixed(2)).toBe('1.99')
    })

    it('returns full precision when no decimals specified', () => {
      expect(BigAmount.fromPrecision('1.5', 8).toFixed()).toBe('1.5')
    })
  })

  describe('toNumber', () => {
    it('returns JS number', () => {
      expect(BigAmount.fromPrecision('1.5', 8).toNumber()).toBe(1.5)
    })
  })

  describe('toSignificant', () => {
    it('handles integers', () => {
      expect(BigAmount.fromPrecision('1234', 8).toSignificant(3)).toBe('1234')
    })

    it('handles values with integer and decimal parts', () => {
      expect(BigAmount.fromPrecision('1.5', 8).toSignificant(3)).toBe('1.5')
    })

    it('trims trailing zeros from significant digits', () => {
      expect(BigAmount.fromPrecision('1.500', 8).toSignificant(4)).toBe('1.5')
    })

    it('handles dust amounts', () => {
      expect(BigAmount.fromBaseUnit('1', 18).toSignificant(4)).toBe('0.000000000000000001')
    })

    it('handles small amounts with leading zeros', () => {
      expect(BigAmount.fromPrecision('0.00012345', 18).toSignificant(4)).toBe('0.0001234')
    })

    it('handles zero', () => {
      expect(BigAmount.fromPrecision('0', 8).toSignificant(4)).toBe('0')
    })
    ...
    it('handles values where integer part exceeds requested digits', () => {
      expect(BigAmount.fromPrecision('12345.6789', 18).toSignificant(3)).toBe('12345')
    })

    it('handles values where integer part uses some significant digits', () => {
      expect(BigAmount.fromPrecision('12.3456', 18).toSignificant(4)).toBe('12.34')
    })
  })
  ...
  describe('JSON serialization', () => {
    it('round-trips through toJSON/fromJSON', () => {
      const original = BigAmount.fromBaseUnit('150000000', 8)
    ...
    it('round-trips ETH through toJSON/fromJSON', () => {
      const original = BigAmount.fromBaseUnit('1000000000000000000', 18)
      const json = original.toJSON()
      const restored = BigAmount.fromJSON(json)
  ...
  describe('isBigAmount', () => {
    it('returns true for BigAmount instances', () => {
      expect(BigAmount.isBigAmount(BigAmount.fromPrecision('1', 8))).toBe(true)
    })
  ...
  describe('real-world patterns', () => {
    it('base unit → fiat display (most common pattern)', () => {
      ...
      const price = 60000
      const fiat = BigAmount.fromBaseUnit(balance, 8).times(price).toFixed(2)
      expect(fiat).toBe('90000.00')
    })

    it('user input → chain storage', () => {
      const userInput = '0.5'
      const baseUnit = BigAmount.fromPrecision(userInput, 8).toBaseUnit()
    ...
    it('inline base unit conversion (replaces manual div/pow)', () => {
      ...
      const price = 60000
      const result = BigAmount.fromBaseUnit(cryptoBaseUnit, 8).times(price).toString()
    ...
    it('contract interaction: BigAmount → BigInt via BigInt(toBaseUnit())', () => {
      const userInput = '1.5'
      expect(BigInt(BigAmount.fromPrecision(userInput, 18).toBaseUnit())).toBe(1500000000000000000n)
    })
    ...
    it('ETH gas calculation', () => {
      ...
      const gasCostEth = BigAmount.fromBaseUnit(gasCostWei, 18)
      expect(gasCostEth.toPrecision()).toBe('0.00063')
    })

    it('ERC-20 approval with exact BigInt (viem pattern)', () => {
      const amount = BigAmount.fromPrecision('100', 6) // 100 USDC
