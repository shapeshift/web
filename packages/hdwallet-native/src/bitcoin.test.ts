...
const BIP44_BENCHMARK_TX_INPUT_TXID =
...
const BIP49_BENCHMARK_TX = benchmarkTx(
  ...
  '23850',
  {
    ...
    tx: {
      vout: [
        undefined,
        { scriptPubKey: { hex: 'a914c9e193b1af9e4349d2ee53b4190e2bd36e59719e87' } },
      ],
    },
  },
...
const BIP84_BENCHMARK_TX_INPUT_TXID =
...
const OP_RETURN_BENCHMARK_TX_INPUT_TXID =
  '799a8923515e0303b15dda074b8341b2cf5efab946fce0d68a6614f32a8fc935'
    ...
    it.each([
      ...
      (_, path, scriptType: any) => {
        const mock = vi
          ...
          .mockReturnValue(
            core.describeUTXOPath(core.bip32ToAddressNList(path), 'Bitcoin', scriptType),
          )
        expect(
          info.btcNextAccountPath({
            coin: 'Bitcoin',
...
describe('NativeBTCWallet', () => {
  ...
  it('does not support p2sh addresses', async () => {
    ...
    ).rejects.toThrowErrorMatchingInlineSnapshot(`[Error: Unsupported script type: p2sh]`)
  })

  it('should not generate addresses for bad script types', async () => {
    const mock = vi
      .spyOn(core, 'getNetwork')
      .mockReturnValue(core.getNetwork('bitcoin', 'p2pkh' as any))
    await expect(
      wallet.btcGetAddress({
        coin: 'Bitcoin',
  ...
  it('should sign a transaction with a locktime correctly', async () => {
    ...
    expect(out?.serializedTx).toBe(
      `${BIP44_BENCHMARK_TX_OUTPUT.slice(0, 86)}${sigHex}${BIP44_BENCHMARK_TX_OUTPUT.slice(
        -156,
        -8,
      )}${locktimeHex}`,
    )
