...
describe('makeMemoWithShortenedFinalAssetAmount', () => {
  it('should be 80 bytes if maxMemoSize is 80', () => {
    ...
    expect(modifiedMemo).toBe(
      ...
      `=:e:${RECEIVE_ADDRESS}:${expectedL1AmountOut}:ss:${affiliateBps}:${EXPECTED_AGGREGATOR_TWO_LAST_CHARS}:${SHORTENED_FINAL_ASSET_CONTRACT_ADDRESS}:95087590209`,
    )
  })
  ...
  it('shortened final asset amount length should be shortened by one if maxMemoSize is Infinity and final asset amount length is under or equal to 17', () => {
    ...
    const quotedMemo = `=:e:${RECEIVE_ADDRESS}:${reallyL1BigAmount}:ss:${affiliateBps}:${EXPECTED_AGGREGATOR_TWO_LAST_CHARS}:${SHORTENED_FINAL_ASSET_CONTRACT_ADDRESS}`
    ...
    const expectedMemo = `=:e:${RECEIVE_ADDRESS}:${reallyL1BigAmount}:ss:${affiliateBps}:${EXPECTED_AGGREGATOR_TWO_LAST_CHARS}:${SHORTENED_FINAL_ASSET_CONTRACT_ADDRESS}:950875902132134101`
  ...
  it('shortened final asset amount should be shortened by 1 if maxMemoSize is Infinity and final asset amount length equal to 18', () => {
    ...
    const expectedMemo = `=:e:${RECEIVE_ADDRESS}:${reallyL1BigAmount}:ss:${affiliateBps}:${EXPECTED_AGGREGATOR_TWO_LAST_CHARS}:${SHORTENED_FINAL_ASSET_CONTRACT_ADDRESS}:9508759021321341201`
  ...
  it('shortened final asset amount should be shortened by 2 if maxMemoSize is Infinity and final asset amount length equal to 19', () => {
    ...
    const expectedMemo = `=:e:${RECEIVE_ADDRESS}:${reallyL1BigAmount}:ss:${affiliateBps}:${EXPECTED_AGGREGATOR_TWO_LAST_CHARS}:${SHORTENED_FINAL_ASSET_CONTRACT_ADDRESS}:9508759021321341202`
  ...
  it('should be under or equal to 220 bytes if maxMemoSize is 220 and memo length is close to 220', () => {
    ...
    expect(modifiedMemo).toBe(
      ...
      `=:e:${BIG_ADDRESS}:${expectedL1AmountOut}:ss:${affiliateBps}:${EXPECTED_AGGREGATOR_TWO_LAST_CHARS}:${SHORTENED_FINAL_ASSET_CONTRACT_ADDRESS}:950875902132105`,
    )
  })
