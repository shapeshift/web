...
export function MixinNativeSuiWalletInfo<TBase extends core.Constructor<core.HDWalletInfo>>(
  Base: TBase,
) {
  ...
  return class MixinNativeSuiWalletInfo extends Base implements core.SuiWalletInfo {
    ...
    suiGetAccountPaths(msg: core.SuiGetAccountPaths): core.SuiAccountPath[] {
...
export function MixinNativeSuiWallet<TBase extends core.Constructor<NativeHDWalletBase>>(
  Base: TBase,
) {
  // eslint-disable-next-line @typescript-eslint/no-shadow
  return class MixinNativeSuiWallet extends Base {
    ...
    async suiSignTx(msg: core.SuiSignTx): Promise<core.SuiSignedTx | null> {
      return this.needsMnemonic(!!this.suiAdapter, async () => {
        const signature = await this.suiAdapter!.signTransaction(
          msg.intentMessageBytes,
          msg.addressNList,
        )
