...
export const RatesRequestSchema = z.object({
  ...
  allowMultiHop: booleanFromString
    .optional()
    .default(true)
...
let swapperModule: Awaited<ReturnType<typeof importSwapperModule>> | null =
  null;
...
export const getRates = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!sellAsset) {
      res
        .status(400)
        .json({ error: `Unknown sell asset: ${sellAssetId}` } as ErrorResponse);
      return;
    }
    if (!buyAsset) {
      res
        .status(400)
        .json({ error: `Unknown buy asset: ${buyAssetId}` } as ErrorResponse);
      return;
    }
    ...
    const enabledSwappers = ENABLED_SWAPPER_NAMES.map((name) => {
      ...
      (name): name is (typeof SwapperName)[keyof typeof SwapperName] =>
        name !== undefined,
    );

    // Fetch rates from all enabled swappers in parallel
    const ratePromises = enabledSwappers.map(
      async (swapperName): Promise<ApiRate | null> => {
        try {
          ...
          return {
            ...
            networkFeeCryptoBaseUnit:
              firstStep.feeData.networkFeeCryptoBaseUnit,
          };
        } catch (error) {
    ...
    const rates: ApiRate[] = results
      .filter(
        (r): r is PromiseFulfilledResult<ApiRate | null> =>
          r.status === "fulfilled",
      )
      .map((r) => r.value)
    ...
    rates.sort((a, b) => {
      ...
      if (!a.error && b.error) return -1;
      try {
        const aBuyAmount = BigInt(
          a.buyAmountCryptoBaseUnit.split(".")[0] ?? "0",
        );
        const bBuyAmount = BigInt(
          b.buyAmountCryptoBaseUnit.split(".")[0] ?? "0",
        );
