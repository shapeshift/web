...
export const Deposit = ({ getDepositGasEstimate }: IdleDepositProps) => {
  ...
  const getApproveGasEstimate = async (): Promise<string | undefined> => {
    ...
    try {
      ...
      console.error('IdleDeposit:getApproveEstimate error:', error)
      toast({
        position: 'top-right',
  ...
  const handleContinue = async (formValues: DepositValues) => {
    ...
    try {
      ...
      console.error('IdleDeposit:handleContinue error:', error)
      toast({
        position: 'top-right',
        description: translate('common.somethingWentWrongBody'),
        title: translate('common.somethingWentWrong'),
