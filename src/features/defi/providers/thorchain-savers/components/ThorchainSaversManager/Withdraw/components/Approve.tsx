...
export const Approve: React.FC<ApproveProps> = ({ accountId, onNext }) => {
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      ...
      // TODO(gomes): remove me both here and in deposit approve, or alternatively re-fetch me
      // This is a result of monkey see monkey do boilerplate copy and paste and doesn't re-set anything at all since this is coming from state already
  ...
  const hasEnoughBalanceForGas = useMemo(
    () =>
      ...
      canCoverTxFees({
        feeAsset,
        estimatedGasCryptoPrecision: bnOrZero(estimatedGasCryptoBaseUnit)
          .div(bn(10).pow(feeAsset.precision))
          .toString(),
        accountId,
      }),
    [accountId, estimatedGasCryptoBaseUnit, feeAsset],
  )
