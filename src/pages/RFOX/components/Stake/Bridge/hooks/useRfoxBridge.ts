...
export const useRfoxBridge = ({
  confirmedQuote,
}: UseRfoxBridgeProps): UseRfoxBridgeReturn => {
  ...
  const sellAsset = useAppSelector((state) =>
    selectAssetById(state, confirmedQuote.sellAssetId),
  );
  const chainId = useMemo(
    () =>
      sellAsset?.chainId && isEvmChainId(sellAsset.chainId)
        ? sellAsset.chainId
        : undefined,
    [sellAsset],
  );
  const buyAsset = useAppSelector((state) =>
    selectAssetById(state, confirmedQuote.buyAssetId),
  );
  const feeAsset = useAppSelector((state) =>
    selectFeeAssetByChainId(
      state,
      fromAssetId(confirmedQuote.sellAssetId).chainId,
    ),
  );
  ...
  const networkFeeCryptoPrecision = useMemo(() => {
    ...
    return fromBaseUnit(
      bnOrZero(
        tradeQuoteQuery.data.unwrap().steps[0].feeData.networkFeeCryptoBaseUnit,
      ),
      sellAsset?.precision ?? 0,
    );
  ...
  const { mutateAsync: handleBridge } = useMutation({
    mutationFn: async () => {
      ...
      if (!arbitrumBridgeApi.getUnsignedEvmTransaction)
  ...
  const { data: tradeStatus } = useQuery({
    queryKey: reactQueries.swapper.arbitrumBridgeTradeStatus(
      l1TxHash,
      sellAsset?.chainId,
    ).queryKey,
    queryFn:
  ...
  useEffect(() => {
    ...
    if (
      tradeStatus.message === "L2 Tx Pending" ||
      tradeStatus.status === TxStatus.Confirmed
    ) {
