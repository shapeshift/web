...
import { selectPendingThorchainLpDepositActions } from '@/state/slices/actionSlice/selectors'
...
export const useThorchainLpDepositActionSubscriber = () => {
        ...
        dispatch(
          actionSlice.actions.upsertAction({
            ...
            transactionMetadata: {
              ...
              message: 'actionCenter.thorchainLp.deposit.complete',
            },
          }),
        )
  ...
  useQueries({
    queries: pendingThorchainLpDepositActions
      .filter(action => {
      ...
      .map(action => ({
        ...
        queryFn: async (): Promise<TxStatus> => {
          const status = await getThorchainTransactionStatus({
            ...
            skipOutbound: true, // LP deposit transactions don't have outbound
          })
