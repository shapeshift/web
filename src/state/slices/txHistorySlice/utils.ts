...
type DeserializedTxIndex = { txAccountSpecifier: string; txid: string; txAddress: string }

export const deserializeTxIndex = (txIndex: TxIndex): DeserializedTxIndex => {
  const [txAccountSpecifier, txid, txAddress] = txIndex.split(UNIQUE_TX_ID_DELIMITER)
