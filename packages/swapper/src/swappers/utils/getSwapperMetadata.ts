import type { SwapperMetadata } from '../../types'
...
export const getSwapperMetadata = <S extends Exclude<SwapperMetadata['swapper'], undefined>>(
  metadata: SwapperMetadata,
  swapper: S,
): Extract<SwapperMetadata, { swapper: S }> => {
  if (metadata.swapper !== swapper) throw new Error(`Expected ${swapper} swap metadata`)
  return metadata as Extract<SwapperMetadata, { swapper: S }>
}
