...
import { bebopApi } from './endpoints'
...
describe('bebop getUnsignedEvmTransaction', () => {
  it('builds from step.transactionData (evm), not bebopTransactionMetadata', async () => {
    const buildCustomApiTx = vi.fn(async (a: any) => a)
    const adapter = {
      buildCustomApiTx,
    }
    const tx = await bebopApi.getUnsignedEvmTransaction!({
      from: '0xFROM',
      stepIndex: 0,
