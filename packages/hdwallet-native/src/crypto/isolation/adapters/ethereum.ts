...
import type { BytesLike, providers, Signature, UnsignedTransaction } from 'ethers'
import { BigNumber } from 'ethers'
import type { Deferrable } from 'ethers/lib/utils.js'
import {
  arrayify,
  computeAddress,
  getAddress,
...
export class SignerAdapter {
  ...
  async getAddress(addressNList: core.BIP32Path): Promise<core.Address> {
    ...
    return computeAddress(
      SecP256K1.UncompressedPoint.from(nodeAdapter.getPublicKey()),
    ) as core.Address
  }
  ...
  async signTransaction(
    ...
    const rawSig = await SecP256K1.RecoverableSignature.signCanonically(
      nodeAdapter.node,
      'keccak256',
      txBuf,
    )
