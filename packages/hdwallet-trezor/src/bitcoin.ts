...
export async function btcSignTx(
  ...
  const outputs: BTCTrezorSignTxOutput[] = msg.outputs.map(output => {
    if (output.addressNList) {
      ...
      (output.addressType as core.BTCOutputAddressType) == core.BTCOutputAddressType.Transfer
    ) {
    ...
    if (output.address) {
      return {
        address:
          msg.coin === 'BitcoinCash' ? bchAddr.toCashAddress(output.address) : output.address,
        amount: output.amount,
