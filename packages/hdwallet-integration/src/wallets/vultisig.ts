...
const cosmosProvider = {
  getOfflineSigner: vi.fn(() => ({
    ...
    getPubkey: vi.fn(() =>
      Promise.resolve(
        new Uint8Array([
          2, 136, 177, 245, 49, 184, 120, 113, 149, 190, 179, 162, 33, 113, 72, 195, 4, 9, 54, 131,
          200, 139, 114, 190, 75, 59, 96, 168, 80, 137, 10, 33, 42,
        ]),
      ),
    ),
  })),
...
export function selfTest(get: () => core.HDWallet): void {
  ...
  describe('Bitcoin', () => {
    ...
    it('supports correct script types', async () => {
      if (!wallet) return
      expect(
        await wallet.btcSupportsScriptType('Bitcoin', core.BTCInputScriptType.SpendWitness),
      ).toEqual(true)
    })
