...
import type { RepayStep } from './repayMachine'
...
const ALL_STEPS: StepConfig[] = [
  { id: 'signing', labelKey: 'chainflipLending.repay.steps.signing' },
  { id: 'confirming', labelKey: 'chainflipLending.repay.steps.confirming' },
]

const STEP_ORDER: RepayStep[] = ALL_STEPS.map(s => s.id)
...
export const RepayStepper = memo(() => {
  const translate = useTranslate()
  const stateValue = RepayMachineCtx.useSelector(s => s.value) as string
  const errorStep = RepayMachineCtx.useSelector(s => s.context.errorStep)

  const getStepStatus = useMemo(() => {
    const currentIndex = STEP_ORDER.indexOf(stateValue as RepayStep)
    const isError = stateValue === 'error'

    return (stepId: RepayStep): StepStatus => {
      const stepIndex = STEP_ORDER.indexOf(stepId)

      if (isError && errorStep === stepId) return 'error'

      if (isError) {
        const errorIndex = STEP_ORDER.indexOf(errorStep as RepayStep)
