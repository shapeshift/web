...
import {
  HypeLabEvent,
  trackHypeLabEvent,
} from "@/lib/hypelab/hypelabSingleton";
  ...
  const executeTrade = useCallback(() => {
    ...
    return new Promise<void>(async (resolve) => {
      ...
      const trackMixpanelEventOnExecute = () => {
        const event =
          hopIndex === 0
            ? MixPanelEvent.TradeConfirm
            : MixPanelEvent.TradeConfirmSecondHop;
      ...
      execution.on(
        ...
        ({ buyTxHash, message, actualBuyAmountCryptoBaseUnit }) => {
          ...
          if (actualBuyAmountCryptoBaseUnit) {
            const freshActiveSwapId = swapSlice.selectors.selectActiveSwapId(
              store.getState(),
            );
            if (freshActiveSwapId) {
              const currentSwap = swapSlice.selectors.selectSwapsById(
                store.getState(),
              )[freshActiveSwapId];
              if (currentSwap) {
                dispatch(
      ...
      execution.on(
        TradeExecutionEvent.Success,
        ({ buyTxHash, actualBuyAmountCryptoBaseUnit }) => {
          if (buyTxHash) {
            txHashReceived = true;
            dispatch(
              tradeQuoteSlice.actions.setSwapBuyTxHash({
      ...
      const skipDeviceDerivation =
        wallet && (isTrezor(wallet) || isGridPlus(wallet));
