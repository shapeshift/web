...
import type { CryptoEngine, ScryptParams } from './types'
...
export class WebCryptoEngine implements CryptoEngine {
  public async decrypt(data: ArrayBuffer, key: ArrayBuffer, iv: ArrayBuffer): Promise<ArrayBuffer> {
    const impKey = await globalThis.crypto.subtle.importKey(
      'raw',
      key,
      { name: 'AES-CBC' },
      false,
  ...
  public async hmac(value: ArrayBuffer, key: ArrayBuffer): Promise<ArrayBuffer> {
    ...
    const impKey = await globalThis.crypto.subtle.importKey('raw', key, signingAlgorithm, false, [
      'sign',
    ])
