...
export function MixinNativeThorchainWalletInfo<TBase extends core.Constructor<core.HDWalletInfo>>(
  Base: TBase,
) {
...
export function MixinNativeThorchainWallet<TBase extends core.Constructor<NativeHDWalletBase>>(
  Base: TBase,
) {
  // eslint-disable-next-line @typescript-eslint/no-shadow
  return class MixinNativeThorchainWallet extends Base {
    ...
    async thorchainSignTx(msg: core.ThorchainSignTx): Promise<core.ThorchainSignedTx | null> {
      return this.needsMnemonic(!!this.#masterKey, async () => {
        ...
        return (await protoTxBuilder).sign(
          adapter.address,
          msg.tx as StdTx,
          adapter,
          signerData,
