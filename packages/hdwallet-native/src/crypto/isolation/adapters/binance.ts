...
export default {
  async create(keyPair: BIP32.Node): Promise<SigningDelegate> {
    return async (tx: Transaction, signMsg?: any): Promise<Transaction> => {
      ...
      const pubKey = (await bnbSdk).crypto.getPublicKey(
        Buffer.from(await keyPair.getPublicKey()).toString('hex'),
      )
      const sig = Buffer.from(
        await SecP256K1.Signature.signCanonically(keyPair, 'sha256', signBytes),
      )
