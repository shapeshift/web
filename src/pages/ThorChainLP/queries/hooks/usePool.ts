...
export const getPool = (
  ...
  return {
    ...
    tvlFiat: BigAmount.fromThorBaseUnit({ value: tvl }).times(bnOrZero(runePrice)).toFixed(),
  }
}
...
export const getFeeStats = (swapsData: ReturnType<typeof selectSwapsData>, runePrice: string) => {
  ...
  const fees24hFiat = BigAmount.fromThorBaseUnit({ value: swaps24h.totalFees })
    .times(runePrice)
    .toFixed()
...
export const getTvlStats = (pool: Pool, tvl24hIntervals: string[], runePrice: string) => {
  ...
  const tvl24hFiat = BigAmount.fromThorBaseUnit({ value: tvl24h }).times(runePrice).toFixed()
...
export const getVolumeStats = (
  ...
  const volume24hFiat = BigAmount.fromThorBaseUnit({ value: swaps24h.totalVolume })
    .times(runePrice)
    .toFixed()
  ...
  const volume7dFiat = BigAmount.fromThorBaseUnit({ value: swapsData.meta.totalVolume })
    .times(runePrice)
    .toFixed()
...
export const usePool = (poolAssetId: string) => {
  ...
  const volumeStats = useMemo(() => {
    ...
    const volumeTotalFiat = BigAmount.fromThorBaseUnit({ value: poolStats.data.swapVolume ?? '0' })
