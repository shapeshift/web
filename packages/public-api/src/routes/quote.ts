...
export const getQuote = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const validSwapperName = Object.values(SwapperName).find(
      (v) => v === swapperName,
    ) as SwapperName | undefined;
    if (!validSwapperName) {
      res
        .status(400)
        .json({ error: `Unknown swapper: ${swapperName}` } as ErrorResponse);
      return;
    }
    ...
    if (!slippage) {
      try {
        slippage =
          getDefaultSlippageDecimalPercentageForSwapper(validSwapperName);
      } catch {
    ...
    const response: QuoteResponse = {
      ...
      buyAmountBeforeFeesCryptoBaseUnit:
        lastStep.buyAmountBeforeFeesCryptoBaseUnit,
      buyAmountAfterFeesCryptoBaseUnit:
        lastStep.buyAmountAfterFeesCryptoBaseUnit,
      affiliateBps: quote.affiliateBps,
      slippageTolerancePercentageDecimal:
        quote.slippageTolerancePercentageDecimal,
