...
export const isUnichainChainAdapter = (adapter: unknown): adapter is EvmChainAdapter => {
  if (!adapter) return false

  const maybeAdapter = adapter as EvmChainAdapter
  if (typeof maybeAdapter.getChainId !== 'function') return false

  return maybeAdapter.getChainId() === unichainChainId
}
...
export const getUnichainTransactionStatus = async (
  ...
  try {
    ...
    console.error('[Unichain] Error getting transaction status:', error)
    return TxStatus.Unknown
  }
}
