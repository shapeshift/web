...
export const useSendDetails = (): UseSendDetailsReturnType => {
  ...
  const estimateFormFees = useCallback((): Promise<FeeDataEstimate<ChainId>> => {
    ...
    const { cryptoAmount, to, sendMax, accountId } = getValues()
  ...
  const handleNextClick = () => history.push(QrCodeRoutes.Confirm)

  const handleSendMax = async () => {
    const fnLogger = moduleLogger.child({ namespace: ['handleSendMax'] })
    fnLogger.trace(
      { asset, feeAsset: feeAsset.assetId, cryptoHumanBalance, fiatBalance },
      'Send Max',
    )
