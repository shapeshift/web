...
import type { ArgonParams, IVaultBackedBy, IVaultFactory, VaultPrepareParams } from './types'
import {
  crypto,
  encoder,
  keyStoreUUID,
...
export class RawVault
  extends Revocable(Object.freeze(class {}))
  implements IVaultBackedBy<Uint8Array>
{
      ...
      (await idb.get<CryptoKey>('machineSeed', await RawVault.#keyStore)) ??
        (await (async () => {
          const machineSeed = await (
            ...
            'raw',
            await (await crypto).getRandomValues(new Uint8Array(32)),
            'HKDF',
            false,
