...
import type { ISealableVaultFactory, IVault, VaultPrepareParams } from '../types'
  ...
  async entriesAsync(): Promise<[string, unknown][]> {
    return await Promise.all(
      Array.from(this.entries()).map(
        async ([k, v]: [string, Promise<unknown>]) => [k, await v] as [string, unknown],
      ),
    )
  }
  ...
  set(key: string, value: unknown | Promise<unknown>): this {
    ...
    super.set(
      ...
      (async () => {
        switch (key) {
          case '#mnemonic':
            return await native.crypto.Isolation.Engines.Default.BIP39.Mnemonic.create(
