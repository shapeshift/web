...
describe('CryptoHelpers', () => {
  ...
  describe('aesEncrypt', () => {
    ...
    it.each([
      ...
      for (const arg of BAD_ARGS) {
        ...
        args.push(arg)
        await expect((helper.aesEncrypt as any)(...(args as any))).rejects.toThrowError(
          `[${name}] is not of type`,
        )
      }
    })
    ...
    it('should work with ArrayBuffers', async () => {
      const key = await helper.makeKey('password', 'email')
      const encrypted = await helper.aesEncrypt(
        core.toArrayBuffer(utils.fromUtf8ToArray(PLAINTEXT_STRING)),
        key,
      )
  ...
  describe('aesDecrypt', () => {
    ...
    it('should fail if the data is incorrect', async () => {
      ...
      await expect(
        helper.aesDecrypt(data, encrypted.iv, encrypted.mac, encrypted.key),
      ).rejects.toThrow('HMAC signature is not valid')
    })
    ...
    it('should fail if the iv is incorrect', async () => {
      ...
      await expect(
        helper.aesDecrypt(encrypted.data, iv, encrypted.mac, encrypted.key),
      ).rejects.toThrow('HMAC signature is not valid')
    })
    ...
    it('should fail if the mac is incorrect', async () => {
      ...
      await expect(
        helper.aesDecrypt(encrypted.data, encrypted.iv, mac, encrypted.key),
      ).rejects.toThrow('HMAC signature is not valid')
    })
    ...
    it('should fail if the key is incorrect', async () => {
      ...
      const encrypted = new CipherString(ENCRYPTED_STRING).toEncryptedObject(key)
      await expect(
        helper.aesDecrypt(encrypted.data, encrypted.iv, encrypted.mac, encrypted.key),
      ).rejects.toThrow('HMAC signature is not valid')
    })
    ...
    it.each([
      ...
      for (const value of BAD_ARGS) {
        const args = new Array(position).fill(dummyArg)
        args.push(value)
        await expect((helper.aesDecrypt as any)(...(args as any))).rejects.toThrowError(
          `[${name}] is not of type`,
        )
      }
    })
  ...
  describe('compare', () => {
    ...
    it('should return false if the hmac results are different sizes', async () => {
      let i = 0
      const mock = vi.spyOn(engine, 'hmac').mockImplementation(async () => {
