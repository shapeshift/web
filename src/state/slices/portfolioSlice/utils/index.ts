...
import {
  fetchPortalsAccount,
  fetchPortalsPlatforms,
  maybeTokenImage,
} from "@/lib/portals/utils";
import {
  assertUnreachable,
  isNativeHDWallet,
  isTrezorHDWallet,
...
export const accountIdToLabel = (accountId: AccountId): string => {
  ...
  switch (chainId) {
    ...
    case btcChainId:
      ...
      if (pubkey.startsWith("zpub") || bech32.decode(pubkey).prefix === "bc")
        return "Segwit Native";
      return "";
    case bchChainId:
      return "Bitcoin Cash";
    case dogeChainId:
...
export const findAccountsByAssetId = (
  ...
  if (result.length === 0 && assetId) {
    return Object.keys(portfolioAccounts).filter(
      (accountId) =>
        fromAssetId(assetId).chainId === fromAccountId(accountId).chainId,
    );
  }
  return result;
};
    ...
    switch (chainNamespace) {
      case CHAIN_NAMESPACE.Evm: {
        ...
        ethAccount.chainSpecific.tokens?.forEach((token) => {
          ...
          if (!isNft(token.assetId) && !assetIds.includes(token.assetId))
            return;

          if (isNft(token.assetId)) {
            if (
              [token.name, token.symbol].some((nftText) =>
                isSpammyNftText(nftText, true),
              )
            )
              return;
          }
          ...
          portfolio.accountBalances.byId[accountId][token.assetId] =
            token.balance;
        });
      ...
      case CHAIN_NAMESPACE.CosmosSdk: {
        ...
        cosmosAccount.chainSpecific.assets?.forEach((asset) => {
          ...
          portfolio.accountBalances.byId[accountId][asset.assetId] =
            asset.amount;
        });
      ...
      case CHAIN_NAMESPACE.Starknet: {
        const starknetAccount =
          account as Account<KnownChainIds.StarknetMainnet>;
        const { chainId, assetId, pubkey } = account;
        const accountId = toAccountId({ chainId, account: pubkey });
      ...
      case CHAIN_NAMESPACE.Ton: {
        const tonAccount = account as Account<KnownChainIds.TonMainnet>;
        const { chainId, assetId, pubkey } = account;
        const accountId = toAccountId({ chainId, account: pubkey });
        ...
        tonAccount.chainSpecific.tokens?.forEach((token) => {
          // don't update portfolio if asset is not in the store
          if (!assetIds.includes(token.assetId)) return;
...
export const checkAccountHasActivity = (account: Account<ChainId>) => {
  ...
  switch (chainNamespace) {
    case CHAIN_NAMESPACE.Evm: {
      ...
      const hasActivity =
        bnOrZero(ethAccount.chainSpecific.nonce).gt(0) ||
        bnOrZero(ethAccount.balance).gt(0);
...
export const isAssetSupportedByWallet = (
  assetId: AssetId,
  wallet: HDWallet,
): boolean => {
  ...
  switch (chainId) {
    ...
    case zecChainId:
      return (
        supportsBTC(wallet) &&
        (isNativeHDWallet(wallet) || isTrezorHDWallet(wallet))
      );
    case cosmosChainId:
  ...
  const totalByAccountId = Object.entries(accountBalances)
    ...
    .reduce<Record<AccountId, BigNumber>>((acc, [accountId, byAssetId]) => {
      const accountTotal = Object.entries(byAssetId)
        ...
        .reduce(
          (innerAcc, [_id, assetBalance]) =>
            innerAcc.plus(bnOrZero(assetBalance)),
          bn(0),
        );
...
export const getHighestUserCurrencyBalanceAccountByAssetId = (
  ...
  if (!assetId) return;
  const accountValueMap = Object.entries(accountIdAssetValues).reduce(
    (acc, [k, v]) => {
...
export const makeAssets = async ({
      ...
      (prev, token) => {
        ...
        if (maybePortalsAsset) {
          ...
          const name = (() => {
            ...
            if (assetSymbols.some((symbol) => !symbol))
              return `${token.name} Pool`;
            ...
            if (assetImages.length === 1) {
              return {
                icon: maybeTokenImage(
                  maybePortalsAsset.image || assetImages[0],
                ),
              };
            }
  ...
  if (chainId === solanaChainId) {
    const account = portfolioAccounts[
      pubkey
    ] as Account<KnownChainIds.SolanaMainnet>;
  ...
  if (chainId === tronChainId) {
    const account = portfolioAccounts[
      pubkey
    ] as Account<KnownChainIds.TronMainnet>;
  ...
  if (chainId === suiChainId) {
    const account = portfolioAccounts[
      pubkey
    ] as Account<KnownChainIds.SuiMainnet>;
  ...
  if (chainId === starknetChainId) {
    const account = portfolioAccounts[
      pubkey
    ] as Account<KnownChainIds.StarknetMainnet>;
