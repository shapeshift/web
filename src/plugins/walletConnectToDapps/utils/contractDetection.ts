...
const KNOWN_CONTRACTS: Record<string, { name: string; type: 'dex' | 'bridge' | 'lending' | 'nft'; icon?: string }> = {
  // Uniswap V3 Router
...
export const detectContractType = (address: string): { name: string; type: string; icon?: string } | null => {
...
export const parseApprovalData = (data: string, chainId: string): { spender: string; amount: string; amountRaw: string; isUnlimited: boolean } | null => {
...
export const extractTokensFromSwapData = (data: string, chainId: string): { inputToken?: string; outputToken?: string } | null => {
  const functionName = getFunctionName(data)
