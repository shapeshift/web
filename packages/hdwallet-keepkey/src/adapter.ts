...
import type { TransportDelegate } from './transport'
...
export type DeviceType<T extends AdapterDelegate<any>> = T extends AdapterDelegate<infer R>
  ? R
  : never
...
export class Adapter<DelegateType extends AdapterDelegate<any>> {
  ...
  async initialize(
    devices?: DeviceType<DelegateType>[],
  ...
  async getDevice(serialNumber?: string): Promise<DeviceType<DelegateType>> {
    ...
    return (
      (
        await Promise.all(
          devices.map(async x =>
            (await this.inspectDevice(x)).serialNumber === serialNumber ? x : null,
          ),
        )
  ...
  async getDevices(): Promise<DeviceType<DelegateType>[]> {
  ...
  async pairRawDevice(
    device: DeviceType<DelegateType>,
    tryDebugLink?: boolean,
  ): Promise<KeepKeyHDWallet> {
    await this.initialize([device], tryDebugLink, true)
    return core.mustBeDefined(
      this.keyring.get<KeepKeyHDWallet>((await this.inspectDevice(device)).serialNumber),
    )
  }
}
