...
export const useSwapper = () => {
  ...
  const [swapperManager] = useState<SwapperManager>(() => {
    ...
    manager.addSwapper(
      SwapperType.Zrx,
      new ZrxSwapper({ web3, adapterManager })
    );
    if (wallet) {
      manager.addSwapper(
        SwapperType.Osmosis,
        new OsmosisSwapper({ wallet, adapterManager })
      );
    }
    return manager;
  });
