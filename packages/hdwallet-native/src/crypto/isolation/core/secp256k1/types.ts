...
import type { Static } from 'funtypes'
import { Literal, Object as Obj, Union } from 'funtypes'
...
import type { ECDSAKey, ECDSARecoverableKey } from './interfaces'
...
const _uncompressedPoint = Object.assign(
  _uncompressedPointBase,
  ByteArray,
  _uncompressedPointStatic,
)
...
const _curvePointStatic = {
  x: (p: CurvePoint): FieldElement =>
    p[0] === 0x04 ? UncompressedPoint.x(p) : CompressedPoint.x(p),
  yIsOdd: (p: CurvePoint): boolean =>
    p[0] === 0x04 ? UncompressedPoint.yIsOdd(p) : CompressedPoint.yIsOdd(p),
  ...
  equal: (lhs: CurvePoint, rhs: CurvePoint) =>
    CurvePoint.yIsOdd(lhs) === CurvePoint.yIsOdd(rhs) &&
    FieldElement.equal(CurvePoint.x(lhs), CurvePoint.x(rhs)),
}
...
const _messageWithPreimage = Object.assign(
  _messageWithPreimageBase,
  ByteArray,
  _messageWithPreimageStatic,
)
...
const _signatureBase = ByteArray(64)
  ...
  .withConstraint(
    x =>
      FieldElement.test(x.slice(32, 64)) || `expected ${x}.s to be within the order of the curve`,
    {
      name: 'Signature.s',
    },
  )
...
const _recoverableSignatureStatic = {
  from: (x: Signature, recoveryParam: RecoveryParam): RecoverableSignature => {
    return checkType(
      ...
      core.compatibleBufferConcat([x, new Uint8Array([recoveryParam])]),
    )
  },
  fromSignature: async (
    x: Signature,
    ...
    for (let recoveryParam: RecoveryParam = 0; recoveryParam < 4; recoveryParam++) {
      ...
      if (
        !CurvePoint.equal(
          publicKey,
          await RecoverableSignature.recoverPublicKey(out, digestAlgorithm, message),
        )
      )
        continue
      return out
  ...
  isLowRecoveryParam: (x: RecoverableSignature) =>
    [0, 1].includes(RecoverableSignature.recoveryParam(x)),
  ...
  signCanonically: async (
    ...
    // This is cryptographically impossible (2^-128 chance) if the key is implemented correctly.
    throw new Error(
      `Unable to generate canonical recoverable signature with public key ${Buffer.from(
        publicKey,
      ).toString('hex')} over message ${Buffer.from(message).toString(
        'hex',
      )}; is your key implementation broken?`,
    )
  },
