...
type ThorUtxoOrCosmosTradeRateOrQuote =
  | ThorTradeUtxoOrCosmosRate
  | ThorTradeUtxoOrCosmosQuote;
...
type MakeThorTradeInput<T extends ThorTradeRateOrQuote> =
  T extends ThorEvmTradeRateOrQuote
...
export const getL1RateOrQuote = async <T extends ThorTradeRateOrQuote>(
  ...
  const sellAssetNativePrecision = getNativePrecision(
    sellAsset.assetId,
    swapperName,
  );
  const buyAssetNativePrecision = getNativePrecision(
    buyAsset.assetId,
    swapperName,
  );
  ...
  const getRouteValues = (
    ...
    return {
      source,
      quote,
      expectedAmountOutThorBaseUnit: bnOrZero(
  ...
  const recommendedMinAmountIn =
    ...
    streamingSwapQuote?.recommended_min_amount_in;
  ...
  const getRouteBuyAmountBeforeFeesCryptoBaseUnit = (
    quote: ThornodeQuoteResponseSuccess,
  ) => {
    const buyAmountBeforeFeesCryptoThorPrecision = bn(
      quote.expected_amount_out,
    ).plus(quote.fees.total);
    return toBaseUnit(
      fromBaseUnit(
        buyAmountBeforeFeesCryptoThorPrecision,
        buyAssetNativePrecision,
      ),
      buyAsset.precision,
    );
  };
  ...
  const getProtocolFees = (quote: ThornodeQuoteResponseSuccess) => {
    ...
    const buyAssetTradeFeeBuyAssetCryptoThorPrecision = bnOrZero(
      quote.fees.total,
    ).minus(quote.fees.affiliate);
  ...
  const getMemo = (route: ThorTradeRoute) => {
    ...
    if (route.isStreaming)
      return assertAndProcessMemo(route.quote.memo, getAffiliate(swapperName));
  ...
  const makeThorTradeRateOrQuote = <U extends ThorTradeRateOrQuote>({
    ...
    const slippage = route.isStreaming
      ? undefined
      : slippageTolerancePercentageDecimal;
    const buyAmountBeforeFeesCryptoBaseUnit =
      getRouteBuyAmountBeforeFeesCryptoBaseUnit(route.quote);
  ...
  switch (chainNamespace) {
    case CHAIN_NAMESPACE.Evm: {
      const { supportsEIP1559 } = input as
        | GetEvmTradeRateInput
        | GetEvmTradeQuoteInput;
      ...
      const maybeRoutes = await Promise.allSettled(
        perRouteValues.map(async (route): Promise<T> => {
          ...
          const networkFeeCryptoBaseUnit =
            adapters.evm.calcNetworkFeeCryptoBaseUnit({
              ...average,
              supportsEIP1559,
      ...
      const routes = maybeRoutes
        .filter(isFulfilled)
        .map((maybeRoute) => maybeRoute.value);
      ...
      if (!routes.length)
        return Err(
          makeSwapErrorRight({
            ...
            cause: maybeRoutes
              .filter(isRejected)
              .map((maybeRoute) => maybeRoute.reason),
          }),
        );
      ...
      const maybeRoutes = await Promise.allSettled(
        perRouteValues.map(async (route): Promise<T> => {
          ...
          const feeData = await (async (): Promise<QuoteFeeData> => {
            ...
            if (!xpub)
              return { networkFeeCryptoBaseUnit: undefined, protocolFees };
    ...
    case CHAIN_NAMESPACE.CosmosSdk: {
      const cosmosChainAdapter = assertGetCosmosSdkChainAdapter(
        sellAsset.chainId,
      );
        ...
        perRouteValues.map(async (route): Promise<T> => {
          ...
          if (input.quoteOrRate === "rate" && input.receiveAddress) {
            try {
              ...
              const contractAddress = contractAddressOrUndefined(
                sellAsset.assetId,
              );
              ...
              const bandwidthPrice =
                params.find((p) => p.key === "getTransactionFee")?.value ??
                1000;
              const energyPrice =
                params.find((p) => p.key === "getEnergyFee")?.value ?? 100;
