...
import type { ISealableVaultFactory, IVault } from './types'
  ...
  describe(name, () => {
    ...
    it('should store a mnemonic', async () => {
      ...
      const mnemonic = (await vault.get(
        '#mnemonic',
      )) as native.crypto.Isolation.Engines.Default.BIP39.Mnemonic
    ...
    describe('ISealable', () => {
      it('should be unwrappable before being sealed', async () => {
        ...
        expect(await unwrapped.get('#mnemonic')).toBe(
      ...
      describe('the unwrapped vault', () => {
        it('should expose the mnemonic via entries()', async () => {
          ...
          const entries = await Promise.all(
            Array.from(unwrapped.entries()).map(async ([k, v]) => [k, await v]),
          )
      ...
      it('should not be unwrappable after being sealed', async () => {
        ...
        expect(() => vault.unwrap()).toThrowErrorMatchingInlineSnapshot(
          `[Error: can't unwrap a sealed vault]`,
        )
      })
    })
    ...
    it('should generate a fresh, random mnemonic when provided with the GENERATE_MNEMONIC magic', async () => {
      ...
      expect(
        ...
        `"02576bde4c55b05886e56eeeeff304006352f935b6dfc1c409f7eae521dbc5558e"`,
      )
