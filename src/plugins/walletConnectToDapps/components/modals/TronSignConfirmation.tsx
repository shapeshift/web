...
export const TronSignConfirmationModal: FC<
  WalletConnectRequestModalProps<TronSignTransactionCallRequest | TronSignMessageCallRequest>
> = ({ onConfirm, onReject, state, topic }) => {
  ...
  const methodSpecificContent: JSX.Element | null = useMemo(() => {
    if (request?.method === TronSigningMethod.TRON_SIGN_TRANSACTION) {
      const transaction = request.params.transaction
      return (
        <Box p={4}>
          <Text
            translation='plugins.walletConnectToDapps.modal.signMessage.contract'
            fontWeight='medium'
            mb={1}
          />
