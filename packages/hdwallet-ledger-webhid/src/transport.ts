...
export async function openTransport(device: HIDDevice): Promise<TransportWebHID> {
  ...
  try {
    ...
    throw new core.WebHIDCouldNotInitialize(
      'Ledger',
...
export function translateCoinAndMethod<
  T extends LedgerTransportCoinType,
  U extends LedgerTransportMethodName<T>,
>(transport: TransportWebHID, coin: T, method: U): LedgerTransportMethod<T, U> {
  switch (coin) {
    case 'Btc': {
    ...
    case null: {
      switch (method) {
        ...
        case 'openApp': {
          const out: LedgerTransportMethod<null, 'openApp'> = ledger.openApp.bind(
            undefined,
            transport as Transport,
          )
          return out as LedgerTransportMethod<T, U>
