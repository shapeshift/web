...
import { getOsmosisInvestor } from 'features/defi/contexts/OsmosisProvider/osmosisInvestorSingleton'
...
export const idleStakingOpportunitiesMetadataResolver = async ({
  ...
  const opportunities = await (async () => {
    const maybeOpportunities = await getOsmosisInvestor().findAll()
    if (maybeOpportunities.length) return maybeOpportunities

    await getOsmosisInvestor().initialize()
    return await getOsmosisInvestor().findAll()
  })()
  ...
  for (const opportunity of opportunities) {
    ...
    lpOpportunitiesById[opportunityId] = baseOpportunity
      ? {
          ...baseOpportunity,
