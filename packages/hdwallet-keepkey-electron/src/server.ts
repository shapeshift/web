import type * as keepkey from '@shapeshiftoss/hdwallet-keepkey'
...
export class Server<
  DeviceType extends object,
  DelegateType extends keepkey.AdapterDelegate<DeviceType>,
> {
  ...
  listen() {
    if (this.delegate.getDevice !== undefined)
      electron.ipcMain.handle(GET_DEVICE, (_, serialNumber?: string) =>
        this.getDevice(serialNumber),
      )
    if (this.delegate.getDevices !== undefined)
      electron.ipcMain.handle(GET_DEVICES, () => this.getDevices())
    electron.ipcMain.handle(GET_TRANSPORT_DELEGATE, (_, handle: string) =>
      this.getTransportDelegate(handle),
    )
