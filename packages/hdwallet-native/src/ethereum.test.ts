...
describe('NativeETHWallet', () => {
  ...
  it('should generate a correct ethereum address', async () => {
    expect(
      await wallet.ethGetAddress({ addressNList: core.bip32ToAddressNList("m/44'/60'/0'/0/0") }),
    ).toBe('0x73d0385F4d8E00C5e6504C6030F47BF6212736A8')
  })

  it('should generate another correct ethereum address at account 1', async () => {
    expect(
      await wallet.ethGetAddress({ addressNList: core.bip32ToAddressNList("m/44'/60'/1'/0/0") }),
    ).toBe('0xeDe5D356020A0aA3DC3fCAf0d899b5c63d7eA4aa')
  })
  ...
  it('should sign a legacy transaction correctly', async () => {
    ...
    `);*/
    expect(parseTransaction(sig!.serialized).from).toEqual(
