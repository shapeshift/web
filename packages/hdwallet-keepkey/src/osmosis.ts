...
export function osmosisGetAccountPaths(
  msg: core.OsmosisGetAccountPaths,
...
export async function osmosisGetAddress(
  ...
  const response = await transport.call(
    Messages.MessageType.MESSAGETYPE_OSMOSISGETADDRESS,
    getAddr,
    {
...
export async function osmosisSignTx(transport: Transport, msg: core.OsmosisSignTx): Promise<any> {
  ...
  return transport.lockDuring(async () => {
    ...
    signTx.setFeeAmount(Number(msg.tx.fee.amount[0].amount))
    signTx.setGas(Number(msg.tx.fee.gas))
