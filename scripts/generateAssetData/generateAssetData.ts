...
const generateAssetData = async () => {
  ...
  if (!REGEN_ALL) {
    try {
      const existingAssetDataJson = JSON.parse(
  ...
  const generatedAssetData = orderedAssetList.reduce<AssetsById>(
    (acc, asset) => {
  ...
  const assetsWithOverridesApplied = Object.entries(
    overrideAssets,
  ).reduce<AssetsById>((prev, [assetId, asset]) => {
...
const readRelatedAssetIndex = () => {
  const relatedAssetIndexJson = JSON.parse(
    fs.readFileSync(RELATED_ASSET_INDEX_PATH, "utf8"),
  );
  return relatedAssetIndexJson;
};

const writeRelatedAssetIndex = (
  relatedAssetIndex: Record<AssetId, AssetId[]>,
) => {
  const filteredOutputData = Object.fromEntries(
    Object.entries(relatedAssetIndex).filter(
  ...
  fs.writeFileSync(
    RELATED_ASSET_INDEX_PATH,
    JSON.stringify(filteredOutputData, null, 2),
  );
};
