...
describe('zrx getUnsignedEvmTransaction', () => {
  it('builds from step.transactionData (evm), not zrxTransactionMetadata', async () => {
    const buildCustomApiTx = vi.fn(async (a: any) => a)
    const adapter = {
      buildCustomApiTx,
    }
    const tx = await zrxApi.getUnsignedEvmTransaction!({
      from: '0xFROM',
      stepIndex: 0,
