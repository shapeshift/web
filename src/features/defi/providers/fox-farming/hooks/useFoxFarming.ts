...
export const useFoxFarming = (contractAddress: string, { skip }: UseFoxFarmingOptions = {}) => {
  ...
  const stake = useCallback(
    async (lpAmount: string) => {
      try {
        if (skip || !farmingAccountId || !isNumber(accountNumber) || !foxFarmingContract || !wallet)
          return
        if (!adapter)
  ...
  const allowance = useCallback(async () => {
    ...
    const _allowance = await uniV2LPContract.allowance(farmingAccountId, contractAddress)
