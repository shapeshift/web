...
import type { SecP256K1 } from '../core'
...
export class OfflineDirectSignerAdapter implements OfflineDirectSigner {
  ...
  static async create(
    isolatedKey: SecP256K1.ECDSAKey,
    prefix: string,
  ): Promise<OfflineDirectSignerAdapter> {
    const pubkey = await isolatedKey.getPublicKey()
    const address = bech32.encode(
      prefix,
      bech32.toWords(
        Digest.Algorithms.ripemd160(Digest.Algorithms.sha256(await isolatedKey.getPublicKey())),
      ),
    )
