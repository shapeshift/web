        ...
        <Flex alignItems='center' justifyContent='space-between' mb={4} key={asset.assetId}>
            ...
            {asset.assetId === thorchainAssetId && (
              <Popover trigger='hover' placement='right-start'>
                ...
                <PopoverContent>
                  <PopoverArrow />
                  <PopoverBody>
                    {translate('walletProvider.trezor.chains.followTheInstructions')}{' '}
                    <Link
                      color='blue.200'
                      href='https://support.trezor.com/hc/en-us/articles/4402987997841-THORChain-RUNE-?docs=true'
                      isExternal
                    >
                      here
                    </Link>{' '}
                    {translate('walletProvider.trezor.chains.toGetTheRuneApp')}
                  </PopoverBody>
                </PopoverContent>
              </Popover>
            )}
          ...
          >
            {walletChainIds.includes(asset.chainId)
              ? translate('walletProvider.trezor.chains.added')
              : translate('walletProvider.trezor.chains.connect')}
          </Button>
        </Flex>
      )),
      ...
      <ModalBody textAlign='left' pb={8}>
        <Box mb={4}>
          <Text
            fontSize='lg'
            fontWeight='bold'
            translation={'walletProvider.trezor.chains.header'}
            mb={2}
          />
          <Text color='text.subtle' translation={'walletProvider.trezor.chains.body'} />
        </Box>
        <Alert status='info' mb={4}>
          <AlertIcon />
          <AlertDescription>
            <Text translation='walletProvider.trezor.connectWarning' />
          </AlertDescription>
        </Alert>
        <Box mb={4}>
          <Text
            fontSize='md'
            fontWeight='bold'
            translation={'walletProvider.trezor.chains.availableChains'}
            mb={2}
          />
          <Box>{chainsRows}</Box>
        </Box>
