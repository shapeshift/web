...
export const nearIntentsApi: SwapperApi = {
  ...
  getUnsignedEvmTransaction: async ({
    ...
    if (!nearIntentsSpecific)
      throw new Error("nearIntentsSpecific is required");
  ...
  getUnsignedUtxoTransaction: ({
    ...
    const satoshiPerByte =
      (feeData.chainSpecific as UtxoFeeData | undefined)?.satsPerByte ?? "0";
  ...
  checkTradeStatus: async ({ config, swap }): Promise<TradeStatus> => {
    ...
    try {
      ...
      const actualBuyAmountCryptoBaseUnit =
        statusResponse.swapDetails?.amountOut;
