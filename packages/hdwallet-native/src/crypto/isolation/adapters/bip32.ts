...
export type BIP32InterfaceAsync = Omit<
  bip32.BIP32Interface,
  'sign' | 'derive' | 'deriveHardened' | 'derivePath'
> &
...
export class BIP32Adapter extends ECPairAdapter implements BIP32InterfaceAsync {
  ...
  protected constructor(
    ...
    super(
      node,
      publicKey,
      networkOrParent instanceof BIP32Adapter ? networkOrParent.network : networkOrParent,
    )
    this.node = node
  ...
  get identifier() {
    return (this._identifier =
      ...
      this._identifier ??
      Buffer.from(
        btccryptoInstance!.hash160(Buffer.from(SecP256K1.CompressedPoint.from(this.publicKey))),
      ))
  }
  get fingerprint() {
