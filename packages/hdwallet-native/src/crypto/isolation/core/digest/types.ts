...
export type AlgorithmName<L extends NonNegativeInteger | undefined = undefined> =
  L extends undefined
    ? keyof typeof AlgorithmLength
    : algorithmNameInner<keyof typeof AlgorithmLength, L>
const algorithmNameStatic = {
  forEach(
    callbackfn: (value: AlgorithmName, index: number, array: AlgorithmName[]) => void,
    thisarg?: any,
  ) {
...
function specificUnverifiedDigest<N extends AlgorithmName = AlgorithmName>(
  name: N,
): Runtype<UnverifiedDigest<N>> {
  return Obj({
    preimage: ByteArray(),
...
function unverifiedDigestBase<N extends AlgorithmName = AlgorithmName>(
  name?: N,
): Runtype<UnverifiedDigest<N>> {
  if (name !== undefined) return specificUnverifiedDigest(name)
  return (Object.keys(AlgorithmName.enumObject) as AlgorithmName[]).reduce(
    <T>(a: Runtype<T>, x: AlgorithmName) => {
...
export const Algorithms = (() => {
  ...
  _initializeAlgorithms(<N extends AlgorithmName>(name: N, fn: Algorithm<N>) => {
    ...
    algorithms[name] = Algorithm(name).enforce((x: ByteArray) => {
      const out = checkType(ByteArray<20 | 32 | 64>(AlgorithmLength[name]), fn(x)) as Partial<
        UnverifiedDigest<N>
      >
      out.preimage = x
      out.algorithm = name
