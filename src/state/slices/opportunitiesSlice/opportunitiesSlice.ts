...
export const opportunitiesApiFactory = () =>
  createApi({
    ...
    endpoints: build => ({
      ...
      getOpportunityUserData: build.query<
        GetOpportunityUserDataOutput,
        GetOpportunityUserDataInput
      >({
        queryFn: async (
          ...
          try {
            ...
            if (!resolver) {
              throw new Error(
                `resolver for ${DefiProvider.FoxFarming}::${defiType} not implemented`,
              )
            }
...
export const opportunitiesApi = opportunitiesApiFactory()
