...
import {
  ...
  TradeQuoteCardError,
  TradeQuoteCardExchangeRate,
  TradeQuoteCardHeader,
  TradeQuoteCardMeta,
  TradeQuoteCardMetaGas,
  TradeQuoteCardMetaPriceImpact,
  TradeQuoteCardMetaSlippage,
...
export const TradeQuoteAdvanced: FC<TradeQuoteProps> = memo(
  ({ isActive, isBest, isFastest, isLowestGas, quoteData, isLoading, onBack }) => {
    ...
    return (
      ...
      >
        <TradeQuoteCardHeader>
          <Flex gap={2}>
            <SwapperIcon withTooltip swapperName={quoteData.swapperName} />
            {quote && (
              <TradeQuoteCardExchangeRate
                buyAsset={buyAsset}
                sellAsset={sellAsset}
                amountReceivedCrypto={totalReceiveAmountCryptoPrecision}
                sellAmountCrypto={sellAmountCryptoPrecision}
              />
            )}
          </Flex>
          <Flex gap={2}>
          ...
          <>
            ...
            <TradeQuoteCardMeta>
              <TradeQuoteCardMetaGas
                ...
                isLoading={isLoading}
              />
              <TradeQuoteCardMetaPriceImpact quote={quote} isLoading={isLoading} />
              <TradeQuoteCardMetaSlippage
                isLoading={isLoading}
                isStreaming={quote.isStreaming}
                quoteSlippagePc={quote.slippageTolerancePercentageDecimal}
                customUserSlippagePc={userSlippagePercentageDecimal}
              />
