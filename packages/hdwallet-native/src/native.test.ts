...
import * as native from './native' // root chain code generated from the 'all all all' seed
...
const STARK_CHAIN_CODE = 'UBS48IuLdtYu2dXcWEAQDfUWmcwYvdSIAPM3VgRd0tI='

const mswMock = mswMockFactory().startServer()
  ...
  it('should produce correct path descriptions', () => {
    ...
    ;[
      ...
      },
      {
        ...
        out: {
          coin: 'Bitcoin',
          verbose: 'Bitcoin Account #0 (Legacy)',
          isKnown: true,
          wholeAccount: true,
        },
      },
    ...
    expect(() => info.describePath({ coin: 'foobar', path: [1, 2, 3] })).toThrowError(
      'Unsupported path',
    )
  })
...
describe('NativeHDWallet', () => {
  ...
  describe('loadDevice', () => {
    it('should load wallet with a mnemonic', async () => {
      ...
      expect(await wallet.isLocked()).toBe(false)
      const testCases: { in: any; out: any }[] = [
        {
        ...
        {
          in: [
            { coin: 'bitcoin', addressNList: [1 + 0x80000000, 2 + 0x80000000], curve: 'secp256k1' },
          ],
          out: [
            {
        ...
        // Note how this produces the same xpub as the path above. This is not intuitive behavior, and is probably a bug.
        {
          in: [
            {
              coin: 'bitcoin',
              addressNList: [1 + 0x80000000, 2 + 0x80000000, 3],
              curve: 'secp256k1',
            },
          ],
          out: [
        ...
        {
          in: [
            { addressNList: [1 + 0x80000000, 2 + 0x80000000, 3 + 0x80000000], curve: 'ed25519' },
          ],
    ...
    it('should throw when attempting to derive a key for a path that is not a child of the explicit path', async () => {
      ...
      for (const params of testCases) {
        await expect(wallet.getPublicKeys(params.in)).rejects.toThrowError(
          'path is not a child of this node',
        )
      }
    })
    ...
    it('should load wallet with a mnemonic and deviceId', async () => {
      ...
      await wallet.loadDevice({
        mnemonic: MNEMONIC,
        deviceId: '0SUnRnGkhCt0T5qk5YmK10v5u+lgHiMMu1R76uD7kHE=',
      })
      expect(await wallet.initialize()).toBe(true)
  ...
  it('should emit MNEMONIC_REQUIRED when initialized without a mnemonic', async () => {
    const wallet = native.create({ deviceId: 'native' })
    const mnemonicRequiredMock = vi.fn(({ message_type }) => {
      expect(message_type).toBe(native.NativeEvents.MNEMONIC_REQUIRED)
    })
    const readyMock = vi.fn(({ message_type }) => {
