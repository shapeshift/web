...
describe('relay getUnsignedEvmTransaction', () => {
  it('builds from step.transactionData (evm), not relayTransactionMetadata', async () => {
    const buildCustomApiTx = vi.fn(async (a: any) => a)
    const adapter = {
      buildCustomApiTx,
    }
    const tx = await relayApi.getUnsignedEvmTransaction!({
      from: '0xFROM',
      stepIndex: 0,
