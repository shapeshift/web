  ...
  configureServer(server) {
    server.middlewares.use((req, res, next) => {
      ...
      if (
        ...
        tryServeCompressed("br", "br", "application/json")
      ) {
        return;
      }
      ...
      if (
        ...
        tryServeCompressed("gzip", "gz", "application/json")
      ) {
        return;
      }
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...Object.fromEntries(
        Object.entries(publicFilesEnvVars).map(([key, value]) => [
          `import.meta.env.${key}`,
          value,
        ]),
      ),
    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: {
          manualChunks: (id) => {
            if (id.includes("node_modules")) {
              ...
              if (
                id.match(
                  /(react-icons|@react-spring|react-datepicker|react-dom)/,
                )
              )
                return "react";
              ...
              if (id.match(/(@sentry|mixpanel|@moralisweb3|moralis)/))
                return "sdk";
              ...
              if (id.includes("bnb-javascript-sdk-nobroadcast"))
                return "bnb-sdk";
            ...
            if (id.includes("packages/unchained-client"))
              return "unchained-client";
            ...
            if (id.includes("packages/chain-adapters"))
              return "z-chain-adapters";
        ...
        onwarn(warning, warn) {
          ...
          if (
            ...
            warning.message.includes(dynamicImport)
          )
            return;
      ...
      sourcemap:
        mode === "development" && !process.env.DEPLOY ? "inline" : true,
      outDir: "build",
    },
  };
});
