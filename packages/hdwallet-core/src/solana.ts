...
export interface SolanaSignRawTx {
  addressNList: BIP32Path
...
export interface SolanaWallet extends SolanaWalletInfo, HDWallet {
  ...
  solanaSignRawTransaction?(msg: SolanaSignRawTx): Promise<SolanaSignedTx | null>
  solanaSignMessage?(msg: SolanaSignMessage): Promise<SolanaSignedMessage | null>
}
...
export function solanaGetAccountPaths(msg: SolanaGetAccountPaths): SolanaAccountPath[] {
    ...
    instruction =>
      new TransactionInstruction({
        keys: instruction.keys.map(key =>
          Object.assign(key, { pubkey: new PublicKey(key.pubkey) }),
        ),
