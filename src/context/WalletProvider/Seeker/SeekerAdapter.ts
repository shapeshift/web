...
export class SeekerHDWallet implements HDWallet {
  ...
  constructor(deviceId: string, pubkey: string) {
  ...
  async solanaSignTx(msg: SolanaSignTx): Promise<SolanaSignedTx | null> {
    const { seekerSignTransaction } = await import(
      '@/context/WalletProvider/Seeker/seekerMessageHandlers'
    )

    const transaction = solanaBuildTransaction(msg, this.pubkey)
    const serializedTx = Buffer.from(transaction.serialize()).toString('base64')

    const result = await seekerSignTransaction(serializedTx)
  ...
  async solanaSendTx(msg: SolanaSignTx): Promise<SolanaTxSignature | null> {
    const { seekerSignAndSendTransaction } = await import(
      '@/context/WalletProvider/Seeker/seekerMessageHandlers'
    )
    ...
    const result = await seekerSignAndSendTransaction(serializedTx)
  ...
  async signTx(transaction: string): Promise<{ signature: string }> {
    ...
    const result = await seekerSignTransaction(transaction)
    if (!result.success || !result.signedTransaction) {
      throw new Error(result.error ?? 'Failed to sign transaction')
    }
  ...
  async signAndBroadcastTx(transaction: string): Promise<{ signature: string }> {
    ...
    const result = await seekerSignAndSendTransaction(transaction)
    if (!result.success || !result.signature) {
  ...
  async broadcastTransaction(hex: string): Promise<string> {
    ...
    const result = await seekerSignAndSendTransaction(hex)
    if (!result.success || !result.signature) {
