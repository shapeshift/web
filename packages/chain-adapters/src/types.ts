...
export type ContractInteraction = Nominal<
  "contract-interaction",
  "ContractInteraction"
>;
export const CONTRACT_INTERACTION: ContractInteraction =
...
export type TxTransfer = Omit<
  unchained.Transfer,
  "components" | "totalValue" | "from" | "to"
> & {
  from: string[];
  to: string[];
  value: string;
...
export type BuildSendApiTxInput<T extends KnownChainIds> = Omit<
  BuildSendTxInput<T>,
  "wallet"
> & {
  from: string;
...
type BuildValidatorTxInput<T extends ChainId> = Omit<
  BuildSendTxInput<T>,
  "to"
> & {
  validator: string;
...
export type BuildUndelegateTxInput<T extends ChainId> =
  BuildValidatorTxInput<T>;

export type BuildClaimRewardsTxInput<T extends ChainId> = Omit<
  BuildValidatorTxInput<T>,
  "value"
>;

export type BuildRedelegateTxInput<T extends ChainId> = Omit<
  BuildSendTxInput<T>,
  "to"
> & {
...
export type BuildDepositTxInput<T extends KnownChainIds> = Omit<
  BuildSendApiTxInput<T>,
  "to"
> & {
  memo: string;
};

type BuildLPTxInput<T extends ChainId> = Omit<
  BuildSendTxInput<T>,
  "to" | "value"
> & {
  poolId: string;
