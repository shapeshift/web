...
import type { TonTransactionParams } from './crypto/isolation/adapters/ton'
import { TonAdapter } from './crypto/isolation/adapters/ton'
import type { NativeHDWalletBase } from './native'

export function MixinNativeTonWalletInfo<TBase extends core.Constructor<core.HDWalletInfo>>(
  Base: TBase,
) {
  ...
  return class MixinNativeTonWalletInfo extends Base implements core.TonWalletInfo {
    ...
    tonGetAccountPaths(msg: core.TonGetAccountPaths): core.TonAccountPath[] {
...
export function MixinNativeTonWallet<TBase extends core.Constructor<NativeHDWalletBase>>(
  Base: TBase,
) {
  // eslint-disable-next-line @typescript-eslint/no-shadow
