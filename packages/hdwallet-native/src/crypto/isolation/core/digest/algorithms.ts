...
import type { Algorithm, AlgorithmName } from './types'
...
export function _initializeAlgorithms(
  register: <N extends AlgorithmName>(name: N, fn: Algorithm<N>) => void,
) {
  ...
  try {
    ...
    register('hash160', (x): any =>
      crypto
        ...
        .update(crypto.createHash('sha256').update(x).digest())
        .digest(),
    )
    ...
    register('keccak256', (x): any =>
      fromWordArray(CryptoJS.SHA3(toWordArray(x), { outputLength: 256 })),
    )
    ...
    register('hash160', (x): any =>
      fromWordArray(CryptoJS.RIPEMD160(CryptoJS.SHA256(toWordArray(x)))),
    )
