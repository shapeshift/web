...
const walletConnectV2Adapter = walletConnectv2.WalletConnectV2Adapter.useKeyring(
  keyring,
...
;(async () => {
  keyring.onAny((name: string[], ...values: any[]) => {
    ...
    log.innerHTML += `<div class="eventEntry">Event: ${name}<br />Values: ${JSON.stringify(
      values,
    )}</div>`
...
window['useTestWallet'] = async function () {
  wallet.loadDevice({
    mnemonic:
      await native.crypto.Isolation.Engines.Dummy.BIP39.Mnemonic.create(testPublicWalletXpubs),
  })
  document.getElementById('#mnemonicModal').className = 'modal'
}
...
const doesRecoveredMatchAddress = (
  typedData: TypedData,
  fromAddress: string,
  signature: string,
): boolean => {
  ...
  return (
    Web3.utils.toChecksumAddress(recoveredAddress) === Web3.utils.toChecksumAddress(fromAddress)
  )
}
    ...
    const outputs = [
      {
        address: (await wallet.btcSupportsScriptType(
          'BitcoinCash',
          core.BTCInputScriptType.CashAddr,
        ))
