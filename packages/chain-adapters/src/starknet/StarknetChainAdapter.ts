          ...
          for (const event of receipt.events) {
            ...
            if (event.keys && event.keys[0] === transferSelector) {
              try {
                ...
                const amountLow = event.data[0] ? BigInt(event.data[0]) : BigInt(0)
                const amountHigh = event.data[1] ? BigInt(event.data[1]) : BigInt(0)
                const amount = amountLow + (amountHigh << BigInt(128))

                // Determine transfer type based on addresses
                const isFromMe = fromAddress.toLowerCase() === pubkey.toLowerCase()
                const isToMe = toAddress.toLowerCase() === pubkey.toLowerCase()
                ...
                console.debug('Error parsing transfer event:', parseError)
              }
            }
          }
        }
