...
const initialState: Preferences = {
  featureFlags: {
    ...
    ThorchainSwapL1ToLongtail:
      getConfig().VITE_FEATURE_THORCHAINSWAP_L1_TO_LONGTAIL,
    ...
    FoxPageFoxFarmingSection:
      getConfig().VITE_FEATURE_FOX_PAGE_FOX_FARMING_SECTION,
    ...
    ThorchainSwapperVolatilityAck:
      getConfig().VITE_FEATURE_THORCHAIN_SWAPPER_ACK,
...
export const preferences = createSlice({
  ...
  reducers: (create) => ({
    ...
    setFeatureFlag: create.reducer(
      (
        state,
        { payload }: { payload: { flag: keyof FeatureFlags; value: boolean } },
      ) => {
        state.featureFlags[payload.flag] = payload.value;
      },
    ),
    setSelectedLocale: create.reducer(
      (state, { payload }: { payload: { locale: string } }) => {
    ...
    setSelectedCurrency: create.reducer(
      (
        state,
        { payload }: { payload: { currency: SupportedFiatCurrencies } },
      ) => {
    ...
    setCurrencyFormat: create.reducer(
      (
        state,
        { payload }: { payload: { currencyFormat: CurrencyFormats } },
      ) => {
    ...
    setWelcomeModal: create.reducer(
      (state, { payload }: { payload: { show: boolean } }) => {
        state.showWelcomeModal = payload.show;
      },
    ),
    setShowConsentBanner: create.reducer(
      (state, { payload }: { payload: boolean }) => {
        state.showConsentBanner = payload;
      },
    ),
    setShowSnapsModal: create.reducer(
      (state, { payload }: { payload: boolean }) => {
        state.showSnapsModal = payload;
      },
    ),
    setSnapInstalled: create.reducer(
      (state, { payload }: { payload: boolean }) => {
        state.snapInstalled = payload;
      },
    ),
    toggleWatchedAssetId: create.reducer(
      (state, { payload }: { payload: AssetId }) => {
        const isWatched = state.watchedAssets.includes(payload);
        if (isWatched) {
          state.watchedAssets = state.watchedAssets.filter(
            (assetId) => assetId !== payload,
          );
        } else {
    ...
    toggleSpamMarkedAssetId: create.reducer(
      (state, { payload }: { payload: AssetId }) => {
        const isSpamMarked = state.spamMarkedAssets.includes(payload);
        if (isSpamMarked) {
          state.spamMarkedAssets = state.spamMarkedAssets.filter(
            (assetId) => assetId !== payload,
          );
        } else {
    ...
    setHomeMarketView: create.reducer(
      (state, { payload }: { payload: HomeMarketView }) => {
        state.selectedHomeView = payload;
      },
    ),
    setQuoteDisplayOption: create.reducer(
      (state, { payload }: { payload: QuoteDisplayOption }) => {
        state.quoteDisplayOption = payload;
      },
    ),
    setHasSeenTcyClaimForWallet: create.reducer(
      (state, { payload }: { payload: WalletId }) => {
    ...
    setShowTopAssetsCarousel: create.reducer(
      (state, { payload }: { payload: boolean }) => {
  ...
  selectors: {
    ...
    selectBalanceThresholdUserCurrency: (state) =>
      state.balanceThresholdUserCurrency,
    ...
    selectHasWalletSeenTcyClaimAlert: (state) =>
      state.hasWalletSeenTcyClaimAlert,
