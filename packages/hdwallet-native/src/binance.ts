...
export function MixinNativeBinanceWalletInfo<TBase extends core.Constructor<core.HDWalletInfo>>(
  Base: TBase,
) {
...
export function MixinNativeBinanceWallet<TBase extends core.Constructor<NativeHDWalletBase>>(
  Base: TBase,
) {
  // eslint-disable-next-line @typescript-eslint/no-shadow
  return class MixinNativeBinanceWallet extends Base {
    ...
    async binanceGetAddress(
      msg: core.BinanceGetAddress & { testnet?: boolean },
    ): Promise<string | null> {
    ...
    async binanceSignTx(
      msg: core.BinanceSignTx & { testnet?: boolean },
      ...
      return this.needsMnemonic(!!this.#masterKey, async () => {
        ...
        const haveAccountNumber =
          !!msg.tx.account_number && Number.isInteger(Number(msg.tx.account_number))
        ...
        const addressFromVerify = this.createBinanceAddress(
        ...
        if (!tx.account_number) {
          const { result, status }: { result: Record<string, unknown>; status: number } =
            core.mustBeDefined(await client.getAccount(addressFrom))
          if (
            !(
              status === 200 &&
              'account_number' in result &&
              typeof result.account_number === 'number'
            )
          )
