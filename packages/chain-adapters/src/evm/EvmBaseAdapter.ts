...
export abstract class EvmBaseAdapter<T extends EvmChainId>
  implements IChainAdapter<T>
{
  ...
  protected constructor(args: EvmBaseAdapterArgs) {
    ...
    if (!this.supportedChainIds.includes(this.chainId)) {
      throw new Error(
        `${this.chainId} not supported. (supported: ${this.supportedChainIds})`,
      );
    }
  }
  ...
  protected assertSupportsChain(
    ...
    const support = (() => {
      switch (
        chainReference ??
        Number(fromChainId(this.chainId).chainReference)
      ) {
        ...
        case Number(
          fromChainId(KnownChainIds.ArbitrumNovaMainnet).chainReference,
        ):
    ...
    if (!support) {
      throw new ChainAdapterError(
        `wallet does not support: ${this.getDisplayName()}`,
        {
  ...
  protected async assertSwitchChain(wallet: ETHWallet) {
    ...
    const adapterChainReference = Number(
      fromChainId(this.chainId).chainReference,
    );
    ...
    try {
      ...
      if (err.code === -32603 && err.message.includes("is not a function"))
        return;
  ...
  async buildSendApiTransaction(
    input: BuildSendApiTxInput<T>,
  ): Promise<SignTx<T>> {
    try {
      const { to, from, value, accountNumber, chainSpecific, customNonce } =
        input;
      const {
        data,
        contractAddress,
      ...
      const fees = ((): NetworkFees => {
        ...
        if (!gasPrice)
          throw new Error("gasPrice is required for non EIP-1559 fees");
  ...
  async getAccount(pubkey: string): Promise<Account<T>> {
    try {
      ...
      const balance = bnOrZero(data.balance).plus(
        bnOrZero(data.unconfirmedBalance),
      );
  ...
  async signMessage(
    signMessageInput: SignMessageInput<ETHSignMessage>,
  ): Promise<string> {
    try {
  ...
  async signTypedData(
    input: SignTypedDataInput<ETHSignTypedData>,
  ): Promise<string> {
    try {
    ...
    try {
      ...
      if (wallet.ethGetAddresses) {
        ...
        return Object.fromEntries(
          accountNumbers.map((accountNumber, i) => [
            accountNumber,
            addresses[i],
          ]),
        );
      ...
      const addresses = await Promise.all(
        accountNumbers.map((accountNumber) =>
          this.getAddress({ accountNumber, wallet }),
        ),
      );
  ...
  async getFeeData(input: GetFeeDataInput<T>): Promise<FeeDataEstimate<T>> {
    try {
      ...
      return {
        fast: {
          txFee: bnOrZero(
            BigNumber.max(fast.gasPrice, fast.maxFeePerGas ?? 0).times(
              gasLimit,
            ),
          ).toFixed(0),
        ...
        average: {
          txFee: bnOrZero(
            BigNumber.max(average.gasPrice, average.maxFeePerGas ?? 0).times(
              gasLimit,
            ),
          ).toFixed(0),
        ...
        slow: {
          txFee: bnOrZero(
            BigNumber.max(slow.gasPrice, slow.maxFeePerGas ?? 0).times(
              gasLimit,
            ),
          ).toFixed(0),
