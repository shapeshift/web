...
import type { EgressStep } from './egressMachine'
...
const ALL_STEPS: StepConfig[] = [
  { id: 'signing', labelKey: 'chainflipLending.egress.steps.signing' },
  { id: 'confirming', labelKey: 'chainflipLending.egress.steps.confirming' },
]

const STEP_ORDER: EgressStep[] = ALL_STEPS.map(s => s.id)
...
export const EgressStepper = memo(() => {
  const translate = useTranslate()
  const stateValue = EgressMachineCtx.useSelector(s => s.value) as string
  const errorStep = EgressMachineCtx.useSelector(s => s.context.errorStep)

  const getStepStatus = useMemo(() => {
    const currentIndex = STEP_ORDER.indexOf(stateValue as EgressStep)
    const isError = stateValue === 'error'

    return (stepId: EgressStep): StepStatus => {
      const stepIndex = STEP_ORDER.indexOf(stepId)

      if (isError && errorStep === stepId) return 'error'

      if (isError) {
        const errorIndex = STEP_ORDER.indexOf(errorStep as EgressStep)
