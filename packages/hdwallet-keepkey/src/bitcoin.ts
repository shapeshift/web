...
function prepareSignTx(
  coin: core.Coin,
  inputs: core.BTCSignTxInputKK[],
  outputs: core.BTCSignTxOutput[],
): any {
  ...
  outputs.forEach((o, k) => {
    ...
    if (output.isChange || output.addressType === core.BTCOutputAddressType.Transfer) {
      ...
      newOutput.setAddressType(
        output.isChange ? Types.OutputAddressType.CHANGE : Types.OutputAddressType.TRANSFER,
      )
  ...
  inputs.forEach(inputTx => {
    ...
    if (coin === 'Dash') {
      ...
      if (prevTx.version === 3 && dip2_type !== 0) {
        ...
        tx.setExtraData(
          core.fromHexString(packVarint(prevTx.extraPayload.length * 2) + prevTx.extraPayload),
        )
      }
...
export async function btcGetAddress(
  ...
  addr.setScriptType(
    translateInputScriptType(msg.scriptType || core.BTCInputScriptType.SpendAddress),
  )
