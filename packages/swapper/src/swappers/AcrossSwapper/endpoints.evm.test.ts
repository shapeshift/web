...
import { acrossApi } from './endpoints'
...
describe('across getUnsignedEvmTransaction', () => {
  it('builds from step.transactionData (evm), not acrossTransactionMetadata', async () => {
    const buildCustomApiTx = vi.fn(async (a: any) => a)
    const adapter = {
      buildCustomApiTx,
    }
    const tx = await acrossApi.getUnsignedEvmTransaction!({
      from: '0xFROM',
      stepIndex: 0,
