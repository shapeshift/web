...
export async function getFirstLedgerDevice(): Promise<USBDevice> {
  ...
  try {
    ...
    throw new core.WebUSBCouldNotInitialize(
      'Ledger',
      String(core.isIndexable(err) ? err.message : err),
    )
  }
}
...
export async function translateCoinAndMethod<
  T extends LedgerTransportCoinType,
  U extends LedgerTransportMethodName<T>,
>(transport: TransportWebUSB, coin: T, method: U): Promise<LedgerTransportMethod<T, U>> {
  switch (coin) {
    case 'Btc': {
    ...
    case 'Thorchain': {
      ...
      const methodInstance =
        thorchain[method as LedgerTransportMethodName<'Thorchain'>].bind(thorchain)
      return methodInstance as LedgerTransportMethod<T, U>
    }
    case 'Cosmos': {
    ...
    case null: {
      switch (method) {
        ...
        case 'openApp': {
          const out: LedgerTransportMethod<null, 'openApp'> = openApp.bind(
            undefined,
            transport as Transport,
          )
          return out as LedgerTransportMethod<T, U>
...
export class LedgerWebUsbTransport extends LedgerTransport {
  ...
  public async call<T extends LedgerTransportCoinType, U extends LedgerTransportMethodName<T>>(
    ...
    try {
      ...
      const methodInstance: LedgerTransportMethod<T, U> = await translateCoinAndMethod(
        transport,
        coin,
        method,
      )
