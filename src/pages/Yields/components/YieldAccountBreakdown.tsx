...
export const YieldAccountBreakdown = ({
  balances,
  yields: _yields,
  assetId,
}: YieldAccountBreakdownProps) => {
  ...
  Object.entries(balances).forEach(([_yieldId, acctBalances]) => {
    acctBalances.forEach(balance => {
      ...
      accountBalances[address].crypto = bnOrZero(accountBalances[address].crypto)
        .plus(balance.amount)
        .toString()
      accountBalances[address].fiat = bnOrZero(accountBalances[address].fiat)
