...
describe('NativeArkeoWallet', () => {
  ...
  it('should generate a correct arkeo address', async () => {
    expect(
      await wallet.arkeoGetAddress({ addressNList: core.bip32ToAddressNList("m/44'/118'/0'/0/0") }),
    ).toBe('arkeo1knuunh0lmwyrkjmrj7sky49uxk3peyzhpse9av')
  })

  it('should generate another correct arkeo address', async () => {
    expect(
      await wallet.arkeoGetAddress({
        addressNList: core.bip32ToAddressNList("m/44'/118'/1337'/123/4"),
      }),
    ).toBe('arkeo14k4dnrrmxdch6nkvvuugsywrgmvlwrqspjur0w')
  })
