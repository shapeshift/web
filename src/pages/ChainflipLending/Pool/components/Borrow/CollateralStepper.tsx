...
import type { CollateralStep } from './collateralMachine'
...
const ALL_STEPS: StepConfig[] = [
  { id: 'signing', labelKey: 'chainflipLending.collateral.steps.signing' },
  { id: 'confirming', labelKey: 'chainflipLending.collateral.steps.confirming' },
]

const STEP_ORDER: CollateralStep[] = ALL_STEPS.map(s => s.id)
...
export const CollateralStepper = memo(() => {
  const translate = useTranslate()
  const stateValue = CollateralMachineCtx.useSelector(s => s.value) as string
  const errorStep = CollateralMachineCtx.useSelector(s => s.context.errorStep)

  const getStepStatus = useMemo(() => {
    const currentIndex = STEP_ORDER.indexOf(stateValue as CollateralStep)
    const isError = stateValue === 'error'

    return (stepId: CollateralStep): StepStatus => {
      const stepIndex = STEP_ORDER.indexOf(stepId)

      if (isError && errorStep === stepId) return 'error'

      if (isError) {
        const errorIndex = STEP_ORDER.indexOf(errorStep as CollateralStep)
