...
import type { BTCSignTx, CosmosSignTx, ETHSignTx, HDWallet, ThorchainSignTx } from '@shapeshiftoss/hdwallet-core';
...
type ChainSpecificAccount<T> = ChainSpecific<T, {
...
export declare enum FeeDataKey {
    Slow = "slow",
    Average = "average",
    Fast = "fast"
}
type ChainSpecificFeeData<T> = ChainSpecific<T, {
...
export type ChainSpecificBuildTxData<T> = ChainSpecific<T, {
...
type ChainSpecificGetFeeDataInput<T> = ChainSpecific<T, {
...
export declare enum ValidAddressResultType {
    Valid = "valid",
    Invalid = "invalid"
}
...
export type ZrxFeeResult = {
    ...
    instant: number;
    low: number;
    source: 'ETH_GAS_STATION' | 'ETHERSCAN' | 'ETHERCHAIN' | 'GAS_NOW' | 'MY_CRYPTO' | 'UP_VEST' | 'GETH_PENDING' | 'MEDIAN' | 'AVERAGE';
    standard: number;
    timestamp: number;
};
...
export declare enum ChainAdapterDisplayName {
    Thorchain = "THORChain",
    Ethereum = "Ethereum",
