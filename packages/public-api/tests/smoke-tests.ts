  ...
  results.push(
    await runTest(`Rates: ${evmPair.name}`, false, async () => {
      ...
      if (!Array.isArray(data.rates))
        throw new Error("Invalid rates response structure");

      const validRates = data.rates.filter(
        (r) =>
          !r.error &&
          r.buyAmountCryptoBaseUnit &&
          r.buyAmountCryptoBaseUnit !== "0",
      );
      if (validRates.length === 0) {
        const swappersWithErrors = data.rates
          ...
          .join(", ");
        console.warn(
          `Warning: No valid rates returned for ${
            evmPair.name
          }. Swappers with errors: ${swappersWithErrors || "none"}`,
        );
      } else {
        console.log(
