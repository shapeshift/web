...
const getRequestFilter =
  (cachedUrls: string[]) => (request: AxiosRequestConfig) =>
...
export const AsyncResultOf = async <T>(
  promise: Promise<T>,
...
export const makeSwapperAxiosServiceMonadic = (service: AxiosInstance) =>
  ...
  >(service, {
    get: (trappedAxios, method: "get" | "post" | "delete") => {
      ...
      return async (
        ...args: [url: string, dataOrConfig?: any, dataOrConfig?: any]
      ) => {
          ...
          .andThen<AxiosResponse>((result) => {
            if (!result.data)
              return Err(
                makeSwapErrorRight({
                  message:
                    "makeSwapperAxiosServiceMonadic: no data was returned",
                  cause: result,
                  code: TradeQuoteError.QueryFailed,
                }),
              );
...
export const getHopByIndex = (
  ...
  if (index > 1) {
    throw new Error(
      "Index out of bounds - Swapper doesn't currently support more than 2 hops.",
    );
  }
...
export const checkSafeTransactionStatus = async ({
  ...
  const { isExecutedSafeTx, isQueuedSafeTx, transaction } =
    await fetchSafeTransactionInfo({
      address,
      chainId,
      fetchIsSmartContractAddressQuery,
      safeTxHash: txHash,
...
export const getInputOutputRate = ({
  ...
  const sellAmountCryptoHuman = fromBaseUnit(
    sellAmountCryptoBaseUnit,
    sellAsset.precision,
  );
  const buyAmountCryptoHuman = fromBaseUnit(
    buyAmountCryptoBaseUnit,
    buyAsset.precision,
  );
...
export const isExecutableTradeQuote = (
  quote: TradeQuote | TradeRate,
): quote is TradeQuote => quote.quoteOrRate === "quote";
...
export const getExecutableTradeStep = (
  ...
  if (!isExecutableTradeStep(step))
    throw Error("Unable to execute a trade rate step");
