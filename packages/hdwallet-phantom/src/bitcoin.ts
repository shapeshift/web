...
import type { PhantomUtxoProvider } from './types'
...
export const btcGetAccountPaths = (msg: core.BTCGetAccountPaths): core.BTCAccountPath[] => {
...
export async function bitcoinGetAddress(_msg: core.BTCGetAddress, provider: any): Promise<string> {
  ...
  const paymentAddress = accounts.find((account: BtcAccount) => account.purpose === 'payment')
    ?.address
